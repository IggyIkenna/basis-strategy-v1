...
class MLService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.api_key = os.getenv(f"BASIS_{environment.upper()}__ML_API_KEY")
    ...
    async def _call_ml_api(self, timestamp: pd.Timestamp) -> Dict[str, float]:
        ...
        try:
            ...
            payload = {"timestamp": timestamp.isoformat(), "signals": self.signals}

            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.model_endpoint, headers=headers, json=payload, timeout=5.0
                ) as response:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for signal in self.signals:
                if signal == "btc_direction":
                    csv_files = list(
                        Path(self.data_dir).glob("ml_data/predictions/btc_predictions*.csv")
                    )
                elif signal == "eth_volatility":
                    csv_files = list(
                        Path(self.data_dir).glob("ml_data/predictions/eth_volatility*.csv")
                    )
                else:
                    continue
