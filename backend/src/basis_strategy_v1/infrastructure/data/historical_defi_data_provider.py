...
Provides CSV-based data for DeFi strategies in backtest mode.
Handles on-chain protocols with 1h granularity.

Key Principles:
- CSV-based data loading for backtest mode
- Position_subscriptions-based data derivation
- 1h granularity for DeFi strategies
...
class HistoricalDeFiDataProvider:
    ...
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize historical DeFi data provider.
        ...
        # If we're in the backend directory, go up one level to find data
        if Path.cwd().name == "backend" and self.data_dir == "data":
            self.data_dir = "../data"
        ...
        logger.info(
            f"HistoricalDeFiDataProvider initialized for {len(self.position_subscriptions)} positions"
        )
    ...
    def _derive_csv_mappings(self) -> Dict[str, str]:
        ...
        for instrument_key in self.position_subscriptions:
            ...
            if not validate_instrument_in_registry(instrument_key):
                logger.warning(f"Position key not in registry: {instrument_key}")
            ...
            if position_type == "BaseToken":
                if instrument == "USDT":
                    ...
                    mappings[
                        f"market_data.prices.{instrument}"
                ...
                if venue != "okx":
                    mappings[
                        f"protocol_data.perp_prices.{price_key}"
                ...
                mappings[
                    f"market_data.funding_rates.{price_key}"
                ...
                # Uppercase key: aUSDT, debtWETH
                base_token = self._get_aave_base_token(instrument)
                mappings[
                    f"protocol_data.aave_indexes.{instrument}"
                ] = f"data/protocol_data/aave/rates/aave_v3_aave-v3-ethereum_{base_token}_rates_*.csv"

            elif position_type == "LST":
                # Oracle pairs: BASE/QUOTE format
                usd_pair = instrument_key_to_oracle_pair(instrument_key, "USD")
                eth_pair = instrument_key_to_oracle_pair(instrument_key, "ETH")

                mappings[
                    f"protocol_data.oracle_prices.{usd_pair}"
                ] = f"data/protocol_data/aave/oracle/{instrument}_USD_oracle_*.csv"
                mappings[
                    f"protocol_data.oracle_prices.{eth_pair}"
                ] = f"data/protocol_data/aave/oracle/{instrument}_ETH_oracle_*.csv"

                # Market prices for DEX/CEX venues
                if venue in ["curve", "uniswapv3"]:
                    market_eth_pair = f"{instrument}/ETH"
                    mappings[
                        f"protocol_data.market_prices.{market_eth_pair}"
                    ] = f"data/market_data/spot_prices/lst_eth_ratios/{venue}_{instrument}WETH_*.csv"

        # Add dust tokens (uppercase)
        for dust_token in self._get_dust_token_positions():
            if dust_token == "EIGEN":
                mappings[
                    f"market_data.prices.{dust_token}"
                ] = "data/market_data/spot_prices/protocol_tokens/binance_EIGENUSDT_1h_2024-10-05_2025-09-30.csv"
            elif dust_token == "ETHFI":
                mappings[
                    f"market_data.prices.{dust_token}"
                ] = "data/market_data/spot_prices/protocol_tokens/binance_ETHFIUSDT_1h_2024-06-01_2025-09-30.csv"
    ...
    def _get_aave_base_token(self, aave_token: str) -> str:
        """Get base token for AAVE token conversion."""
        # Map AAVE tokens to their base tokens
        if aave_token.startswith("a") or aave_token.startswith("debt"):
            # Remove 'a' or 'debt' prefix
            base_token = aave_token[1:] if aave_token.startswith("a") else aave_token[4:]
            # Map to actual base token names in our data
            if base_token == "USDT":
                return "USDT"
            elif base_token in ["WETH", "ETH"]:
                return "WETH"
            elif base_token in ["weETH", "wstETH"]:
                return base_token  # These have their own rate files
        return aave_token

    def _is_seasonal_rewards_enabled(self) -> bool:
        """Check if seasonal rewards are enabled in config."""
        return (
            self.config.get("component_config", {})
            .get("position_monitor", {})
            .get("settlement", {})
            .get("seasonal_rewards_enabled", False)
        )

    def _get_dust_token_positions(self) -> List[str]:
        """Get dust token positions from position_subscriptions."""
        dust_tokens = []
        for instrument_key in self.position_subscriptions:
            venue, position_type, instrument = instrument_key.split(":")
            # Check for dust tokens (EIGEN, ETHFI) from KING unwrapping
            if (
                venue == "wallet"
                and position_type == "BaseToken"
                and instrument in ["EIGEN", "ETHFI"]
            ):
                dust_tokens.append(instrument)
        return dust_tokens

    def get_data(self, timestamp: pd.Timestamp) -> Dict[str, Any]:
        """Get data snapshot at timestamp using standardized structure with caching."""
        try:
    ...
    def _load_fresh_data(self, timestamp: pd.Timestamp) -> Dict[str, Any]:
        """Load fresh data from CSV files for a given timestamp."""
        data = {
            "timestamp": timestamp,
    ...
    def _is_critical_data_key(self, data_key: str) -> bool:
        ...
        - Market prices for base tokens (USDT, WETH, etc.)
        - AAVE indexes for aTokens and debtTokens
        - Oracle prices for LST tokens
        ...
        non_critical_patterns = [
            "execution_data.gas_costs",
            "execution_data.execution_costs",
            "protocol_data.staking_rewards",
            "protocol_data.seasonal_rewards",
        ]
    ...
    def clear_cache(self):
        ...
        logger.info("Historical DeFi data cache cleared")
    ...
    def _load_csv_value(self, csv_path: str, timestamp: pd.Timestamp) -> float:
        ...
        if pd.isna(nearest_idx):
            ...
            if "execution_cost" in csv_path:
                logger.warning(
                    f"No execution cost data found for {csv_path} at {timestamp}, using default value"
                )
                return 0.0
            else:
    ...
    def _extract_value_from_row(self, row: pd.Series, csv_path: str) -> Any:
        ...
        """
        try:
            # AAVE rates files - extract liquidityIndex for aTokens
            if "aave" in csv_path.lower() and "rates" in csv_path.lower():
                if "liquidityIndex" in row.index:
                    return row["liquidityIndex"]
                elif "liquidity_index" in row.index:
                    return row["liquidity_index"]
                else:
                    # Fallback to first numeric column
                    for col in row.index:
                        if col != "timestamp" and pd.api.types.is_numeric_dtype(type(row[col])):
                            return row[col]
                    raise ValueError(f"No numeric column found in AAVE rates file: {csv_path}")
            ...
            # Oracle prices files - extract price column
            elif "oracle" in csv_path.lower():
                # Look for price-related columns
                price_cols = [
                    col for col in row.index if "price" in col.lower() and col != "timestamp"
                ]
                if price_cols:
                    ...
                    for col in row.index:
                        if col != "timestamp" and pd.api.types.is_numeric_dtype(type(row[col])):
                            return row[col]
                    raise ValueError(f"No price column found in oracle file: {csv_path}")

            # Funding rates files - extract rate column
            elif "funding" in csv_path.lower():
                # Look for rate-related columns
                rate_cols = [
                    col for col in row.index if "rate" in col.lower() and col != "timestamp"
                ]
                if rate_cols:
    ...
    def _resolve_csv_path(self, csv_path: str) -> Optional[str]:
        ...
        # Prefer files with proper datetime indexes (avoid integer timestamp files)
        # Check each file to see if it has a proper datetime index
        datetime_files = []
        for match in matches:
            try:
                # Quick check: read first few lines to see if index is datetime
                import pandas as pd

                sample_df = pd.read_csv(match, nrows=5, index_col=0, parse_dates=True, comment="#")
                # Check if index is actually a DatetimeIndex, not just has 'datetime' in the string
                if isinstance(sample_df.index, pd.DatetimeIndex):
                    datetime_files.append(match)
            except:
                continue

        # Use datetime files if available, otherwise fall back to most recent
        if datetime_files:
            return max(datetime_files, key=lambda x: Path(x).stat().st_mtime)
        else:
    ...
    def get_timestamps(self, start_date: str, end_date: str) -> List[pd.Timestamp]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(filtered_timestamps)} timestamps between {start_date} and {end_date}"
            )
