...
class HistoricalDeFiDataProvider:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"HistoricalDeFiDataProvider initialized for {len(self.position_subscriptions)} positions")
    ...
    def _derive_csv_mappings(self) -> Dict[str, str]:
        ...
        for position_key in self.position_subscriptions:
            ...
            if position_type == 'BaseToken':
                ...
                mappings[f'protocol_data.aave_indexes.{instrument}'] = \
                ...
                eth_pair = position_key_to_oracle_pair(position_key, 'ETH')
                
                mappings[f'protocol_data.oracle_prices.{usd_pair}'] = \
                    f'data/protocol_data/aave/oracle/{instrument}_USD_oracle_*.csv'
                mappings[f'protocol_data.oracle_prices.{eth_pair}'] = \
        ...
        for dust_token in self._get_dust_token_positions():
            if dust_token == 'EIGEN':
                mappings[f'market_data.prices.{dust_token}'] = \
    ...
    def _is_seasonal_rewards_enabled(self) -> bool:
        ...
        return self.config.get('component_config', {}).get('position_monitor', {}).get('settlement', {}).get('seasonal_rewards_enabled', False)
    ...
    def _get_dust_token_positions(self) -> List[str]:
        ...
        for position_key in self.position_subscriptions:
            ...
            if venue == 'wallet' and position_type == 'BaseToken' and instrument in ['EIGEN', 'ETHFI']:
