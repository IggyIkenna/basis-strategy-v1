"""
BTC Basis Data Provider - Canonical Architecture Implementation

Mode-specific data provider for BTC basis trading strategy implementing the canonical
...
class BTCBasisDataProvider(BaseDataProvider):
    """Data provider for BTC basis mode implementing canonical architecture."""
    
    def __init__(self, execution_mode: str, config: Dict[str, Any]):
        """Initialize BTC basis data provider."""
    ...
    def _validate_data_requirements(self, data_requirements: List[str]) -> None:
        ...
        if missing_requirements:
            raise ValueError(
                f"BTCBasisDataProvider cannot satisfy requirements: {missing_requirements}. "
    ...
    def get_data(self, timestamp: pd.Timestamp) -> Dict[str, Any]:
        """
        Return standardized data structure for BTC basis mode.
        ...
        return {
            'market_data': {
                'prices': {
                    'BTC': self._get_btc_price(timestamp),
                    'USDT': 1.0,  # Always 1.0
    ...
    def load_data(self):
        ...
        self._load_data()
        logger.info("Data loaded for BTC basis mode (compatibility method)")
    ...
    def _load_data(self):
        ...
        self.data = {
            ...
            'funding_rates': pd.DataFrame({
                'timestamp': pd.date_range('2024-01-01', periods=100, freq='H', tz='UTC'),
                'binance_btc': [0.0001] * 100,
                'bybit_btc': [0.0001] * 100,
                'okx_btc': [0.0001] * 100
            }),
            'perp_prices': pd.DataFrame({
                'timestamp': pd.date_range('2024-01-01', periods=100, freq='H', tz='UTC'),
                'binance_btc_perp': [45000.0] * 100,
                'bybit_btc_perp': [45000.0] * 100,
                'okx_btc_perp': [45000.0] * 100
            }),
        ...
        logger.info("Data loaded for BTC basis mode")
    ...
    def _get_funding_rates(self, timestamp: pd.Timestamp) -> Dict[str, float]:
        ...
        if 'funding_rates' in self.data:
            ...
            closest_idx = df['timestamp'].sub(timestamp).abs().idxmin()
            return {
                'BTC_binance': df.loc[closest_idx, 'binance_btc'],
                'BTC_bybit': df.loc[closest_idx, 'bybit_btc'],
                'BTC_okx': df.loc[closest_idx, 'okx_btc']
            }
        return {
            'BTC_binance': 0.0001,
            'BTC_bybit': 0.0001,
    ...
    def _get_perp_prices(self, timestamp: pd.Timestamp) -> Dict[str, float]:
        ...
        if 'perp_prices' in self.data:
            ...
            closest_idx = df['timestamp'].sub(timestamp).abs().idxmin()
            return {
                'BTC_binance': df.loc[closest_idx, 'binance_btc_perp'],
                'BTC_bybit': df.loc[closest_idx, 'bybit_btc_perp'],
                'BTC_okx': df.loc[closest_idx, 'okx_btc_perp']
            }
        return {
            'BTC_binance': 45000.0,
            'BTC_bybit': 45000.0,
