...
class LiveDataProvider:
    ...
    def _load_live_config(self) -> LiveDataConfig:
        ...
        return LiveDataConfig(
            # CEX API Keys
            binance_spot_api_key=os.getenv("BASIS_DEV__CEX__BINANCE_SPOT_API_KEY")
            or os.getenv("BASIS_STAGING__CEX__BINANCE_SPOT_API_KEY")
            or os.getenv("BASIS_PROD__CEX__BINANCE_SPOT_API_KEY"),
            binance_spot_secret=os.getenv("BASIS_DEV__CEX__BINANCE_SPOT_SECRET")
            or os.getenv("BASIS_STAGING__CEX__BINANCE_SPOT_SECRET")
            or os.getenv("BASIS_PROD__CEX__BINANCE_SPOT_SECRET"),
            binance_futures_api_key=os.getenv("BASIS_DEV__CEX__BINANCE_FUTURES_API_KEY")
            or os.getenv("BASIS_STAGING__CEX__BINANCE_FUTURES_API_KEY")
            or os.getenv("BASIS_PROD__CEX__BINANCE_FUTURES_API_KEY"),
            binance_futures_secret=os.getenv("BASIS_DEV__CEX__BINANCE_FUTURES_SECRET")
            or os.getenv("BASIS_STAGING__CEX__BINANCE_FUTURES_SECRET")
            or os.getenv("BASIS_PROD__CEX__BINANCE_FUTURES_SECRET"),
            bybit_api_key=os.getenv("BASIS_DEV__CEX__BYBIT_API_KEY")
            or os.getenv("BASIS_STAGING__CEX__BYBIT_API_KEY")
            or os.getenv("BASIS_PROD__CEX__BYBIT_API_KEY"),
            bybit_secret=os.getenv("BASIS_DEV__CEX__BYBIT_SECRET")
            or os.getenv("BASIS_STAGING__CEX__BYBIT_SECRET")
            or os.getenv("BASIS_PROD__CEX__BYBIT_SECRET"),
            okx_api_key=os.getenv("BASIS_DEV__CEX__OKX_API_KEY")
            or os.getenv("BASIS_STAGING__CEX__OKX_API_KEY")
            or os.getenv("BASIS_PROD__CEX__OKX_API_KEY"),
            okx_secret=os.getenv("BASIS_DEV__CEX__OKX_SECRET")
            or os.getenv("BASIS_STAGING__CEX__OKX_SECRET")
            or os.getenv("BASIS_PROD__CEX__OKX_SECRET"),
            okx_passphrase=os.getenv("BASIS_DEV__CEX__OKX_PASSPHRASE")
            or os.getenv("BASIS_STAGING__CEX__OKX_PASSPHRASE")
            or os.getenv("BASIS_PROD__CEX__OKX_PASSPHRASE"),
            # DeFi API Keys
            alchemy_api_key=os.getenv("BASIS_DOWNLOADERS__ALCHEMY_API_KEY"),
            etherscan_api_key=os.getenv("BASIS_DOWNLOADERS__ETHERSCAN_API_KEY"),
            # RPC Endpoints
            ethereum_rpc_url=os.getenv("BASIS_DEV__ALCHEMY__RPC_URL")
            or os.getenv("BASIS_STAGING__ALCHEMY__RPC_URL")
            or os.getenv("BASIS_PROD__ALCHEMY__RPC_URL"),
            # Cache settings
            cache_ttl_seconds=int(os.getenv("BASIS_CACHE__TTL__MARKET_DATA", 60)),
            max_retries=int(os.getenv("BASIS_LIVE_DATA__MAX_RETRIES", 3)),
            timeout_seconds=int(os.getenv("BASIS_LIVE_DATA__TIMEOUT_SECONDS", 10)),
        )
    ...
    async def validate_live_data_connections(self) -> Dict[str, Any]:
        ...
        if validation_results["overall_status"] == "healthy":
            ...
            logger.error(
                f"❌ Live data validation failed for mode '{self.mode}': {validation_results['errors']}"
            )
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=self.live_config.timeout_seconds)
        )
        return self
    ...
    async def _get_binance_funding_rate(self, asset: str) -> float:
        ...
        async with self.session.get(url, params=params) as response:
            if response.status == 200:
                ...
                raise Exception(f"Binance funding rate API error: {response.status}")
    ...
    async def _get_bybit_funding_rate(self, asset: str) -> float:
        ...
        async with self.session.get(url, params=params) as response:
            if response.status == 200:
                ...
                raise Exception(f"Bybit funding rate API error: {response.status}")
    ...
    async def _get_okx_funding_rate(self, asset: str) -> float:
        ...
        async with self.session.get(url, params=params) as response:
            if response.status == 200:
                ...
                raise Exception(f"OKX funding rate API error: {response.status}")
    ...
    async def _get_from_cache(self, key: str) -> Optional[Dict]:
        ...
        if key in self._price_cache:
            ...
            if (
                last_update
                and (datetime.now(timezone.utc) - last_update).seconds
