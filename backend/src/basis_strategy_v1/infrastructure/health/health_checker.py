...
class SystemHealthCheck(ComponentHealthCheck):
    ...
    def _perform_health_check(self) -> tuple[str, str, Dict[str, Any]]:
        ...
        details["cpu"] = {"percent_used": cpu_percent, "count": psutil.cpu_count()}
...
class DataProviderHealthCheck(ComponentHealthCheck):
    ...
    def _perform_health_check(self) -> tuple[str, str, Dict[str, Any]]:
        ...
        # Debug logging
        logger.debug(
            f"Data provider health check - BASIS_DATA_DIR: {os.getenv('BASIS_DATA_DIR')}, data_dir: {data_dir}"
        )
        logger.debug(f"Current working directory: {os.getcwd()}")
        logger.debug(
            f"All BASIS_ env vars: {[k for k in os.environ.keys() if k.startswith('BASIS_')]}"
        )
