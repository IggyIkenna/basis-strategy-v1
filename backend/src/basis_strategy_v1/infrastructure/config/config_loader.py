...
class ConfigLoader:
    ...
    def _load_base_config(self) -> Dict[str, Any]:
        """Load base configuration from YAML files only."""
        # JSON configuration loading has been removed per separation of concerns
        # All configuration now comes from YAML files and environment variables
        # Status: PENDING

        # JSON configuration loading removed - all config now comes from YAML files
        # and environment variables per separation of concerns
        return {}
    ...
    def _load_all_share_class_configs(self) -> Dict[str, ShareClassConfig]:
        ...
        if not share_classes_dir.exists():
            raise ConfigurationValidationError(
                f"Missing share_classes directory: {share_classes_dir}"
            )
            ...
            try:
                with open(share_class_file, "r") as f:
                    ...
                    if share_class_config_dict is None:
                        raise ConfigurationValidationError(
                            f"Empty share class config file: {share_class_file}"
                        )
                    ...
                    logger.debug(
                        f"✅ Loaded and validated share class config: {share_class_file.stem}"
                    )
        ...
        if not share_classes:
            raise ConfigurationValidationError(
                f"No share class configurations found in {share_classes_dir}"
            )
    ...
    def get_mode_config(self, mode: str) -> ModeConfig:
        ...
        if mode not in self._config_cache["modes"]:
            raise KeyError(
                f"Mode configuration '{mode}' not found. Available modes: {list(self._config_cache['modes'].keys())}"
            )
    ...
    def get_venue_config(self, venue: str) -> VenueConfig:
        ...
        if venue not in self._config_cache["venues"]:
            raise KeyError(
                f"Venue configuration '{venue}' not found. Available venues: {list(self._config_cache['venues'].keys())}"
            )
    ...
    def get_share_class_config(self, share_class: str) -> ShareClassConfig:
        ...
        if share_class not in self._config_cache["share_classes"]:
            raise KeyError(
                f"Share class configuration '{share_class}' not found. Available share classes: {list(self._config_cache['share_classes'].keys())}"
            )
    ...
    def get_complete_config(
        self, mode: str = None, venue: str = None, scenario: str = None
    ) -> Dict[str, Any]:
