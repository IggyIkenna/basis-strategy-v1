...
class ConfigValidator:
    ...
    def validate_all(self) -> ValidationResult:
        ...
        if self.warnings:
            logger.warning(
                f"⚠️ Configuration validation completed with {len(self.warnings)} warnings"
            )
    ...
    def _validate_base_configs(self):
        ...
        logger.debug(
            "✅ JSON configs eliminated - infrastructure handled by environment variables and hardcoded defaults"
        )
    ...
    def _validate_environment_variables(self):
        ...
        if execution_mode == "live":
            ...
            if self.environment in ["dev", "staging", "prod"]:
                ...
                if placeholder_vars:
                    self.warnings.append(
                        f"Environment variables with placeholder values: {', '.join(placeholder_vars)}"
                    )
            ...
            self.errors.append(
                f"Invalid BASIS_EXECUTION_MODE: {execution_mode}. Must be 'backtest' or 'live'"
            )
    ...
    def _validate_share_class_configs(self):
        ...
        expected_share_classes = ["usdt_stable.yaml", "eth_directional.yaml"]
    ...
    def _validate_business_logic(self):
        ...
        for mode_name, config in mode_configs.items():
            ...
            self._validate_share_class_strategy_compatibility(
                config, mode_name, share_class_configs
            )
    ...
    def _validate_mode_business_logic(self, config: Dict[str, Any], mode_name: str):
        ...
        if not (0.0 <= position_deviation_threshold <= 1.0):
            self.errors.append(
                f"Mode {mode_name}: position_deviation_threshold must be between 0.0 and 1.0, got {position_deviation_threshold}"
            )
        ...
        if "use_flash_loan" in config:
            self.errors.append(
                f"Mode {mode_name}: use_flash_loan parameter is deprecated and should be removed"
            )
        if "unwind_mode" in config:
            self.errors.append(
                f"Mode {mode_name}: unwind_mode parameter is deprecated and should be removed"
            )
        ...
        if hedge_allocation:
            ...
            if abs(total_allocation - 1.0) > 0.01:
                self.warnings.append(
                    f"Mode {mode_name}: hedge_allocation sums to {total_allocation}, expected 1.0"
                )
        ...
        for component in required_components:
            if component not in component_config:
                self.errors.append(
                    f"Mode {mode_name}: Missing required component_config.{component}"
                )
            else:
    ...
    def _validate_component_config(self, component: str, config: Dict[str, Any], mode_name: str):
        ...
        if component == "risk_monitor":
            ...
            if "enabled_risk_types" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.risk_monitor missing enabled_risk_types"
                )
            if "risk_limits" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.risk_monitor missing risk_limits"
                )
            ...
            if "exposure_currency" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.exposure_monitor missing exposure_currency"
                )
            if "track_assets" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.exposure_monitor missing track_assets"
                )
            if "conversion_methods" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.exposure_monitor missing conversion_methods"
                )
            ...
            if "attribution_types" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.pnl_monitor missing attribution_types"
                )
            if "reporting_currency" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.pnl_monitor missing reporting_currency"
                )
            if "reconciliation_tolerance" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.pnl_monitor missing reconciliation_tolerance"
                )
            ...
            if "strategy_type" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_manager missing strategy_type"
                )
            if "actions" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_manager missing actions"
                )
            if "rebalancing_triggers" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_manager missing rebalancing_triggers"
                )
            if "position_calculation" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_manager missing position_calculation"
                )
            ...
            if "supported_actions" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.execution_manager missing supported_actions"
                )
            ...
            if "result_types" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.results_store missing result_types"
                )
            if "balance_sheet_assets" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.results_store missing balance_sheet_assets"
                )
            if "pnl_attribution_types" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.results_store missing pnl_attribution_types"
                )
            ...
            if "timeout" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_factory missing timeout"
                )
            if "max_retries" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_factory missing max_retries"
                )
            if "validation_strict" not in config:
                self.errors.append(
                    f"Mode {mode_name}: component_config.strategy_factory missing validation_strict"
                )

    def _validate_share_class_strategy_compatibility(
        self, mode_config: Dict[str, Any], mode_name: str, share_class_configs: Dict[str, Any]
    ):
        ...
        if not share_class_config:
            self.errors.append(
                f"Mode {mode_name}: Share class '{mode_share_class}' not found in share class configs"
            )
            return
        ...
        if mode_name not in supported_strategies:
            self.errors.append(
                f"Mode {mode_name}: Strategy not supported by share class '{mode_share_class}'. Supported strategies: {supported_strategies}"
            )
        ...
        if mode_asset and share_class_currency:
            ...
            if "usdt" in mode_name.lower() and share_class_currency != "USDT":
                self.errors.append(
                    f"Mode {mode_name}: USDT strategy should use USDT share class, not {share_class_currency}"
                )
            ...
            if "eth" in mode_name.lower() and share_class_currency != "ETH":
                ...
                if (
                    ...
                    and share_class_currency == "USDT"
                ):
                    pass  # Allow market neutral ETH strategies to use USDT share class
                else:
                    self.errors.append(
                        f"Mode {mode_name}: ETH strategy should use ETH share class, not {share_class_currency}"
                    )
        ...
        if mode_leverage and not share_class_leverage:
            self.errors.append(
                f"Mode {mode_name}: Leverage enabled but share class '{mode_share_class}' does not support leverage"
            )
    ...
    def _create_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "share_classes_available": len(
                list((self.base_dir / "configs" / "share_classes").glob("*.yaml"))
            ),
            "backtest_scenarios": len(
                list((self.base_dir / "configs" / "scenarios" / "backtest").glob("*.yaml"))
            ),
            "live_scenarios": len(
                list((self.base_dir / "configs" / "scenarios" / "live").glob("*.yaml"))
            ),
            "environment_variables_set": len(
                [k for k, v in os.environ.items() if k.startswith("BASIS_") and v]
            ),
    ...
    def _validate_live_trading_variables(self):
        ...
        for var in boolean_vars:
            ...
            if value not in ["true", "false"]:
                self.errors.append(
                    f"Invalid {var}: {live_trading_vars[var]}. Must be 'true' or 'false'"
                )

        # Log current live trading configuration
        logger.info(
            f"Live trading configuration: enabled={live_trading_vars['BASIS_LIVE_TRADING__ENABLED']}, "
