...
class ConfigManager:
    ...
    Environment variable integration has been standardized:
    - BASIS_ENVIRONMENT: Environment (dev, staging, prod)
    - BASIS_EXECUTION_MODE: Execution mode (backtest, live)
    - BASIS_DEPLOYMENT_MODE: Deployment style (local, docker, k8s)
    """
    ...
    def create_component(self, component_type: str, config: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self._handle_error(
                "CONFIG-MGR-007", f"Failed to create component {component_type}: {e}"
            )
            return None
    ...
    def _load_environment_variables(self) -> Dict[str, str]:
        ...
        if environment == "dev":
            ...
            logger.error(
                f"CONFIG-MGR-002: Unknown environment: {environment}. Must be dev, staging, or prod"
            )
    ...
    def _validate_environment_variable(self, var_name: str, value: str) -> str:
        ...
        if var_name in ["BASIS_DEBUG", "DATA_VALIDATION_STRICT"]:
            if value.lower() not in ["true", "false"]:
                ...
                raise ValueError(
                    f"Invalid boolean value for {var_name}: {value}. Must be 'true' or 'false'"
                )
            return value.lower()
        ...
        if var_name == "BASIS_ENVIRONMENT":
            if value not in ["dev", "staging", "prod"]:
                ...
                raise ValueError(
                    f"Invalid environment value for {var_name}: {value}. Must be dev, staging, or prod"
                )
            return value
        ...
        if var_name == "BASIS_DEPLOYMENT_MODE":
            if value not in ["local", "docker", "staging", "prod"]:
                logger.error(
                    f"CONFIG-MGR-004: Invalid deployment mode value for {var_name}: {value}"
                )
                raise ValueError(
                    f"Invalid deployment mode value for {var_name}: {value}. Must be local, docker, staging, or prod"
                )
            return value
        ...
        if var_name == "BASIS_DEPLOYMENT_MACHINE":
            if value not in ["local_mac", "gcloud_linux_vm", "staging_server", "prod_server"]:
                logger.error(
                    f"CONFIG-MGR-004: Invalid deployment machine value for {var_name}: {value}"
                )
                raise ValueError(
                    f"Invalid deployment machine value for {var_name}: {value}. Must be local_mac, gcloud_linux_vm, staging_server, or prod_server"
                )
            return value
        ...
        if var_name == "BASIS_DATA_MODE":
            if value not in ["csv", "db"]:
                ...
                raise ValueError(
                    f"Invalid data mode value for {var_name}: {value}. Must be csv or db"
                )
            return value
        ...
        if var_name == "BASIS_EXECUTION_MODE":
            if value not in ["backtest", "live"]:
                logger.error(
                    f"CONFIG-MGR-004: Invalid execution mode value for {var_name}: {value}"
                )
                raise ValueError(
                    f"Invalid execution mode value for {var_name}: {value}. Must be backtest or live"
                )
            return value
        ...
        if var_name == "BASIS_LOG_LEVEL":
            if value not in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
                ...
                raise ValueError(
                    f"Invalid log level value for {var_name}: {value}. Must be DEBUG, INFO, WARNING, ERROR, or CRITICAL"
                )
            return value
        ...
        if var_name == "HEALTH_CHECK_INTERVAL":
            if not value.endswith(("s", "m", "h")):
                ...
                raise ValueError(
                    f"Invalid duration format for {var_name}: {value}. Must end with s, m, or h"
                )
            return value
    ...
    def get_mode_config(self, mode_name: str) -> Dict[str, Any]:
        ...
        if mode_name not in self.config_cache["modes"]:
            raise KeyError(
                f"Mode '{mode_name}' not found. Available modes: {list(self.config_cache['modes'].keys())}"
            )
    ...
    def get_venue_config(self, venue_name: str) -> Dict[str, Any]:
        ...
        if venue_name not in self.config_cache["venues"]:
            raise KeyError(
                f"Venue '{venue_name}' not found. Available venues: {list(self.config_cache['venues'].keys())}"
            )
    ...
    def get_share_class_config(self, share_class_name: str) -> Dict[str, Any]:
        ...
        if share_class_name not in self.config_cache["share_classes"]:
            raise KeyError(
                f"Share class '{share_class_name}' not found. Available share classes: {list(self.config_cache['share_classes'].keys())}"
            )
    ...
    def log_performance_metric(
        ...
        try:
            ...
            self.log_structured_event(
                timestamp, "performance_metric", "INFO", message, "ConfigManager", metric_data
            )
        except Exception as e:
    ...
    def log_error(
        ...
        try:
            ...
            self.log_structured_event(
                timestamp, "error", "ERROR", str(error), "ConfigManager", error_data
            )
        except Exception as e:
    ...
    def log_warning(
        ...
        try:
            timestamp = pd.Timestamp.now(tz="UTC")
            self.log_structured_event(
                timestamp, "warning", "WARNING", message, "ConfigManager", data, correlation_id
            )
        except Exception as e:
    ...
    def _load_base_config(self) -> Dict[str, Any]:
        ...
        logger.info(
            "✅ Base configuration eliminated - infrastructure handled by environment variables"
        )
    ...
    def _load_all_share_class_configs(self) -> Dict[str, Any]:
        ...
        for yaml_file in share_classes_dir.glob("*.yaml"):
            try:
                ...
                logger.error(
                    f"CONFIG-MGR-005: Share class configuration validation failed: {str(e)}"
                )
                raise ValueError(
                    f"Share class configuration validation failed for {yaml_file}: {e}"
                )
...
def load_scenario_config(scenario: str) -> Dict[str, Any]:
    ...
    logger.warning(
        f"Scenario configuration requested for '{scenario}' - scenarios have been eliminated"
    )
    return {}
