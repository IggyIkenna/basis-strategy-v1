...
class ChartGenerator:
    ...
    def __init__(self, theme: str = "plotly_white", config: Optional[Dict[str, Any]] = None):
        ...
        self.generate_venue_balance_chart = output_config.get("generate_venue_balance_chart", True)
        self.generate_token_balance_chart = output_config.get("generate_token_balance_chart", True)
        self.generate_ltv_time_series = output_config.get("generate_ltv_time_series", True)
        ...
        self.generate_margin_time_series = output_config.get("generate_margin_time_series", False)
        self.generate_eth_move_analysis = output_config.get("generate_eth_move_analysis", False)
        ...
        logger.info(
            f"ChartGenerator initialized with output flags: "
    ...
    def generate_all_charts(
        self, results: Dict[str, Any], request_id: str, strategy_name: str, output_dir: Path
    ) -> Dict[str, Path]:
        """
        ...
        """
        try:
            logger.info(f"Generating charts for {request_id} ({strategy_name})")
            ...
            charts_to_generate.append(("equity_curve", self._generate_equity_curve))
            charts_to_generate.append(("pnl_attribution", self._generate_pnl_attribution))
            ...
            charts_to_generate.append(("fee_breakdown", self._generate_fee_breakdown))
            charts_to_generate.append(("metrics_summary", self._generate_metrics_summary))

            # Implemented additional charts requested by UI
            charts_to_generate.append(("ltv_ratio", self._generate_ltv_ratio))
            charts_to_generate.append(("balance_venue", self._generate_balance_venue))
            charts_to_generate.append(("balance_token", self._generate_balance_token))
            charts_to_generate.append(("margin_health", self._generate_margin_health))
            charts_to_generate.append(("exposure", self._generate_exposure))

            # Always generate trade history (placeholder)
            charts_to_generate.append(("trade_history", self._generate_trade_history))
            ...
            if self.generate_eth_move_analysis:
                charts_to_generate.append(("eth_move_analysis", self._generate_eth_move_analysis))

            for chart_name, generator_func in charts_to_generate:
                try:
                    chart_path = output_dir / f"{request_id}_{strategy_name}_{chart_name}.html"
                    fig = generator_func(results, request_id, strategy_name)
                    if fig:
                        fig.write_html(str(chart_path))
                        chart_paths[chart_name] = chart_path
                        logger.debug(f"Generated {chart_name} chart: {chart_path}")
                except Exception as e:
                    logger.warning(f"Failed to generate {chart_name} chart: {e}")

            # Save data to CSV if enabled
            if self.save_data_csv:
                try:
                    self._save_data_csv(results, request_id, strategy_name, output_dir)
            ...
            try:
                dashboard_path = output_dir / f"{request_id}_{strategy_name}_dashboard.html"
                dashboard_fig = self._generate_dashboard(results, request_id, strategy_name)
                if dashboard_fig:
            ...
            logger.info(f"Generated {len(chart_paths)} charts for {request_id}")
            return chart_paths
    ...
    def _generate_equity_curve(
        ...
        try:
            ...
            if equity_curve is None:
                logger.warning(
                    "  Equity curve data is None - will use fallback",
                    extra={"error_code": "CHART_ERROR_001"},
                )
            elif not equity_curve:
                logger.warning(
                    "  No equity curve data - will use fallback",
                    extra={"error_code": "CHART_ERROR_002"},
                )
            else:
            ...
            if not equity_curve or equity_curve is None:
                logger.warning(
                    ...
                    extra={"error_code": "CHART_ERROR_003"},
                )
                # Fallback to simple curve if no time series data
                # Use backtest start/end dates instead of today's date
                ...
                if start_date and end_date:
                    ...
                    # Ultimate fallback to today if no backtest dates available
    ...
    def _price_for_token(self, token: str, timestamp) -> float:
        ...
        if parent == "ETH":
            try:
                return float(self.config.get("backtest", {}).get("default_eth_price", 3000.0))
            except Exception:
                return 3000.0
        return 0.0

    def _snapshot_positions(self, snapshot: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            for snap in equity_curve:
                ...
                for key, amount in positions.items():
                    ...
                    if is_perp:
                        perps_usd[parent] = perps_usd.get(parent, 0.0) + abs(usd_val)
                    elif is_debt:
                        debts_usd[parent] = debts_usd.get(parent, 0.0) + abs(usd_val)
                    else:
                        assets_usd[parent] = assets_usd.get(parent, 0.0) + usd_val
                # Compute net exposure per parent
                parents = set(
                    list(assets_usd.keys()) + list(debts_usd.keys()) + list(perps_usd.keys())
                )
                for parent in parents:
                    net = (
                        assets_usd.get(parent, 0.0)
                        - debts_usd.get(parent, 0.0)
                        - perps_usd.get(parent, 0.0)
                    )
    ...
    def _generate_balance_token(
        ...
        try:
            ...
            for snap in equity_curve:
                ...
                for parent in parents:
                    net = assets_usd.get(parent, 0.0) - debts_usd.get(parent, 0.0)
            ...
            for parent, values in balances_by_token.items():
                fig.add_trace(go.Scatter(x=timestamps, y=values, mode="lines", name=parent))
            fig.update_layout(
        ...
        try:
            ...
            for snap in equity_curve:
                ...
                for key, amount in positions.items():
                    ...
                    if "DEBT" in key or key.endswith("_PERP") or "_PERP_" in key:
                        continue
                    price = self._price_for_token(token, ts)
                    if price == 0.0:
                        continue
                    usd_val = amount * price
                    venue_totals[venue] = venue_totals.get(venue, 0.0) + usd_val
            ...
            for venue, values in balances_by_venue.items():
                fig.add_trace(go.Scatter(x=timestamps, y=values, mode="lines", name=venue))
            fig.update_layout(
    ...
    def _generate_ltv_ratio(
        ...
        try:
            ...
            fig = go.Figure(go.Scatter(x=timestamps, y=ltv_values, mode="lines", name="LTV"))
            fig.update_layout(
    ...
    def _generate_margin_health(
        ...
        try:
            ...
            for snap in equity_curve:
                ...
                health = ((usd_balances + used_margin) / perps_usd) if perps_usd > 0 else 0.0
                health_values.append(health)
            fig = go.Figure(
                go.Scatter(x=timestamps, y=health_values, mode="lines", name="Margin Health")
            )
            fig.update_layout(
    ...
    def _generate_pnl_attribution(
        ...
        try:
            ...
            if "lending" in component_summaries:
                ...
                pnl_data["Lending Interest"] = lending_summary.get("total_interest_usd", 0)
                pnl_data["Lending Fees"] = -lending_summary.get("gas_costs_paid", 0)

            if "staking" in component_summaries:
                staking_summary = component_summaries["staking"]
                pnl_data["Staking Yield"] = staking_summary.get("total_yield_usd", 0)
                pnl_data["Staking Fees"] = -staking_summary.get("gas_costs_paid", 0)

            if "basis_trading" in component_summaries:
                basis_summary = component_summaries["basis_trading"]
                pnl_data["Funding Payments"] = basis_summary.get("total_funding_usd", 0)
                pnl_data["Trading Fees"] = -basis_summary.get("total_fees_usd", 0)

            if not pnl_data:
                # Fallback to simple total return
                total_return = results.get("total_return", 0)
                initial_capital = results.get("initial_capital", 1)
                pnl_data["Total PnL"] = float(total_return) * float(initial_capital)
    ...
    def _generate_component_performance(
        self, results: Dict[str, Any], request_id: str, strategy_name: str
    ) -> Optional[go.Figure]:
        ...
        try:
            ...
            for component_name, summary in component_summaries.items():
                ...
                # Extract fees
                fees.append(summary.get("gas_costs_paid", 0) + summary.get("total_fees_usd", 0))

                # Extract volumes
                volumes.append(
                    summary.get("total_supplied_usd", 0) + summary.get("total_staked_usd", 0)
                )

            # Returns by component
            fig.add_trace(
                go.Bar(
                    x=components, y=returns, name="Returns", marker_color=self.colors["success"]
                ),
                row=1,
                col=1,
            )
            ...
            # Fees by component
            fig.add_trace(
                go.Bar(x=components, y=fees, name="Fees", marker_color=self.colors["danger"]),
                row=1,
                col=2,
            )
            ...
            # Volume by component
            fig.add_trace(
                go.Bar(x=components, y=volumes, name="Volume", marker_color=self.colors["info"]),
                row=2,
                col=1,
            )
    ...
    def _generate_balance_evolution(
        self, results: Dict[str, Any], request_id: str, strategy_name: str
    ) -> Optional[go.Figure]:
    ...
    def _generate_fee_breakdown(
        ...
        try:
            ...
            fig.update_layout(title=f"Fee Breakdown - {strategy_name}", template=self.theme)
    ...
    def _generate_metrics_summary(
        ...
        try:
            ...
            pnl_usd = (
                max(0.0, final_value - initial_capital)
                if (initial_capital and final_value)
                else (total_return * initial_capital)
            )
            pnl_bps = ((pnl_usd / total_traded_usd) * 10000.0) if total_traded_usd > 0 else 0.0
            ...
            metrics = {
                ...
                "Components Used": len(results.get("components_used", [])),
            }
            ...
            y_positions = list(range(len(metrics)))[::-1]  # Reverse for top-to-bottom
    ...
    def _generate_dashboard(
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(x=dates, y=equity_values, mode="lines", name="Portfolio Value"),
                row=1,
                col=1,
            )
            ...
            if component_summaries:
                ...
                pnl_values = [
                    summary.get("net_profit", 0) for summary in component_summaries.values()
                ]

                fig.add_trace(go.Bar(x=components, y=pnl_values, name="PnL"), row=1, col=2)

            # Add component returns
            if component_summaries:
                returns = [
                    summary.get("total_interest_usd", summary.get("total_yield_usd", 0))
                    for summary in component_summaries.values()
                ]

                fig.add_trace(go.Bar(x=components, y=returns, name="Returns"), row=2, col=1)
            ...
            if fee_data:
                fig.add_trace(
                    go.Pie(
                        labels=list(fee_data.keys()), values=list(fee_data.values()), name="Fees"
                    ),
                    row=2,
                    col=2,
                )
    ...
    def _save_data_csv(
        ...
        try:
            ...
            if equity_curve:
                ...
                csv_path = output_dir / f"{request_id}_{strategy_name}_equity_curve.csv"
            ...
            component_summaries = results.get("component_summaries", {})
            if component_summaries:
                df_components = pd.DataFrame.from_dict(component_summaries, orient="index")
                csv_path = output_dir / f"{request_id}_{strategy_name}_component_summaries.csv"
            ...
            trades = results.get("trades", []) or results.get("trade_history", [])
            if trades:
                ...
                csv_path = output_dir / f"{request_id}_{strategy_name}_trades.csv"
            ...
            if event_log:
                ...
                def flatten_entry(e: Dict[str, Any]) -> Dict[str, Any]:
                    ...
                    flat["positions"] = ";".join([f"{k}={v}" for k, v in positions.items()])
                    # event_data summarized
                    event_data = e.get("event_data")
                    flat["event_data"] = str(event_data) if event_data is not None else None
                    return flat

                rows = [flatten_entry(e) for e in event_log]
                df_events = pd.DataFrame(rows)
                csv_path = output_dir / f"{request_id}_{strategy_name}_event_log.csv"
