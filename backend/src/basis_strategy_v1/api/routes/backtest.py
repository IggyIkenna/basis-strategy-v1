...
async def list_backtests(
    http_request: Request, service: BacktestService = Depends(get_backtest_service)
    ...
    try:
        ...
        return StandardResponse(success=True, data=backtests)

    except Exception as e:
        logger.error(
            "Failed to list backtests", correlation_id=correlation_id, error=str(e), exc_info=True
        )
...
async def run_backtest(
    ...
    try:
        ...
        logger.info(
            "Backtest queued successfully", correlation_id=correlation_id, request_id=request_id
        )
        ...
        logger.error("Invalid backtest request", correlation_id=correlation_id, error=str(e))
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(
            "Failed to start backtest", correlation_id=correlation_id, error=str(e), exc_info=True
        )
...
async def get_backtest_status(
    request_id: str, http_request: Request, service: BacktestService = Depends(get_backtest_service)
    ...
    try:
        ...
        if status.get("status") == "not_found":
            logger.warning(
                "Backtest not found", correlation_id=correlation_id, request_id=request_id
            )
...
async def cancel_backtest(
    ...
    try:
        ...
        if not cancelled:
            ...
            raise HTTPException(
                status_code=404, detail=f"Backtest {request_id} not found or already completed"
            )

        logger.info(
            "Backtest cancelled successfully", correlation_id=correlation_id, request_id=request_id
        )

        return StandardResponse(
            success=True, data={"message": f"Backtest {request_id} cancelled successfully"}
        )
