...
async def start_live_trading(
    ...
    try:
        ...
        logger.warning(
            "Live trading validation failed",
            correlation_id=correlation_id,
            error=str(e)
        )
...
async def stop_live_trading(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="Live trading strategy not found or already stopped")
...
async def emergency_stop_live_trading(
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            data={"message": "Live trading emergency stopped", "request_id": request_id, "reason": reason},
            correlation_id=correlation_id
        )
...
async def list_running_strategies(
    ...
    try:
        ...
        return StandardResponse(
            success=True,
            data=LiveTradingStrategiesResponse(
        ...
        logger.error(
            "Failed to list running strategies",
            correlation_id=correlation_id,
            error=str(e)
        )
