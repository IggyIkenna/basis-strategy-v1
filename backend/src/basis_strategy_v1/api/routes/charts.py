...
async def list_charts(
    ...
    try:
        logger.info(
            "Listing available charts", correlation_id=correlation_id, request_id=request_id
        )
                ...
                for item in results_dir.iterdir():
                    if item.is_dir() and item.name.startswith(request_id + "_"):
                        ...
                        if len(parts) == 2:
                            ...
                            logger.info(
                                "Inferred strategy name for listing",
        ...
        def chart_exists(name: str) -> bool:
            ...
            candidate_paths = [
                FilePath(
                    f"results/{request_id}_{strategy_name}/{request_id}_{strategy_name}_{name}.html"
                )
                if strategy_name
                else None,
                ...
                FilePath(
                    f"odum-basis-strategy-v1/results/scenarios/{strategy_name}/{strategy_name}_{name}.html"
                )
                if strategy_name
                else None,
            ]
        ...
        if not charts_found:
            ...
            if result:
                charts_found = {
                    name: f"/api/v1/results/{request_id}/charts/{name}" for name in default_charts
                }
            else:
                # No result and no files discovered
                raise HTTPException(
                    status_code=404, detail=f"No charts found for backtest {request_id}"
                )
...
async def get_chart(
    ...
    try:
        ...
        if result:
            ...
            results_dir = FilePath("results")
            logger.info(
                f"Result not found for {request_id}, trying to infer strategy name from directories"
            )
            for item in results_dir.iterdir():
                ...
                if item.is_dir() and item.name.startswith(request_id):
                    ...
                    if len(parts) == 2:
                        strategy_name = parts[1]
                        logger.info(
                            f"Inferred strategy name '{strategy_name}' from directory: {item.name}"
                        )
                        break
        ...
        chart_paths = [
            ...
            FilePath(
                f"results/{request_id}_{strategy_name}/{request_id}_{strategy_name}_{chart_name}.html"
            ),
            ...
            FilePath(
                f"odum-basis-strategy-v1/results/scenarios/{strategy_name}/{strategy_name}_{chart_name}.html"
            ),
        ]
        ...
        if not chart_content:
            try:
                logger.info(
                    "Attempting glob-based fallback to locate chart file",
                    correlation_id=correlation_id,
                    request_id=request_id,
                    chart_name=chart_name,
                )
                ...
                if matched_path and matched_path.exists():
                    with open(matched_path, "r", encoding="utf-8") as f:
                        chart_content = f.read()
                    logger.info(
                        f"Successfully read chart content from glob match ({len(chart_content)} chars)"
                    )
            except Exception as e:
        ...
        if not chart_content:
            raise HTTPException(
                status_code=404, detail=f"Chart '{chart_name}' not found for backtest {request_id}"
            )

        return HTMLResponse(
            content=chart_content, headers={"Content-Type": "text/html; charset=utf-8"}
        )
