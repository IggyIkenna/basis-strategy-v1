...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(
            f"❌ Failed to get environment check there is a valid env file being loaded during deployment: {e}"
        )
        raise RuntimeError(
            f"Failed to get environment check there is a valid env file being loaded during deployment: {e}"
        )

    logger.info("Starting Basis Strategy API", environment=environment, version="2.0.0")
    ...
    try:
        ...
        logger.info(
            f"✅ Config manager initialized - execution mode: {execution_mode}, data mode: {data_mode}"
        )
...
def create_application() -> FastAPI:
    ...
    app = FastAPI(
        ...
        openapi_tags=[
            ...
            {"name": "capital", "description": "Capital management (deposits and withdrawals)"},
            {"name": "strategies", "description": "Strategy information and management"},
            {"name": "backtest", "description": "Backtest execution and management"},
            {"name": "live_trading", "description": "Live trading execution and management"},
            {"name": "results", "description": "Result retrieval (filesystem-only)"},
        ],
    )
    ...
    app.include_router(live_trading.router, prefix="/api/v1/live", tags=["live_trading"])
    app.include_router(strategies.router, prefix="/api/v1/strategies", tags=["strategies"])
    app.include_router(results.router, prefix="/api/v1/results", tags=["results"])
    app.include_router(charts.router, prefix="/api/v1/results", tags=["charts"])
