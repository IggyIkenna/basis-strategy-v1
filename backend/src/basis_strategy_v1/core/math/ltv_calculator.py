...
class LTVCalculator:
    ...
    def get_max_ltv_for_collateral_mix(
        ...
        for token, value in collateral_composition.items():
            ...
            if is_borrowing_eth and lookup_token in ['wstETH', 'weETH']:
                ...
                logger.debug(f"Standard mode: {token} safe LTV: {token_ltv:.3f} (market risk protection)")
            
            # Calculate weighted contribution
            weight = value / total_collateral_value
            weighted_ltv += token_ltv * weight
        
        mode_type = "E-mode" if is_borrowing_eth and any(token in ['wstETH', 'weETH'] for token in collateral_composition.keys()) else "Standard mode"
    ...
    @staticmethod
    def get_max_borrowing_capacity(
        collateral_value: Decimal,
        max_ltv: Decimal
    ) -> Decimal:
    ...
    def get_emode_eligibility(
        ...
        return (collateral_token in eth_collateral_tokens and 
                debt_token in eth_debt_tokens)
    ...
    @staticmethod
    def calculate_leverage_headroom(
        current_ltv: Decimal,
        safe_ltv: Decimal
    ) -> Decimal:
