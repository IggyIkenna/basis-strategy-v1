...
class MarginCalculator:
    ...
    def estimate_basis_margin_requirements(
        ...
        if share_class == 'ETH':
            ...
            basis_leverage_factor = Decimal(str(strategy_config['basis_leverage_factor']))  # Fail-fast if missing
    ...
    def calculate_margin_for_exposure(
        ...
        initial_margin_pct = Decimal(str(risk_config['bybit_initial_margin_pct']))  # Fail-fast if missing
        price_buffer_pct = Decimal(str(risk_config['price_buffer_pct']))  # Fail-fast if missing
        margin_buffer = Decimal(str(risk_config['basis_trade_margin_buffer']))  # Fail-fast if missing
    ...
    def calculate_margin_health(
        ...
        margin_ratio = estimated_margin / total_position_value if total_position_value > 0 else Decimal("999")
