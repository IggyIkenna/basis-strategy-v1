...
class MetricsCalculator:
    ...
    def calculate_metrics(
        portfolio, initial_capital: Decimal, timestamp: datetime
    ) -> Dict[str, any]:
        """
        ...
        metrics = {
            ...
            "balance_count": sum(len(balances) for balances in portfolio.balances.values()),
            "debt_count": sum(len(debts) for debts in portfolio.debts.values()),
            "position_count": len(portfolio.positions),
        }

        # Calculate net exposure
        total_assets = sum(
            sum(balances.values(), Decimal("0")) for balances in portfolio.balances.values()
        )
        total_debts = sum(sum(debts.values(), Decimal("0")) for debts in portfolio.debts.values())
