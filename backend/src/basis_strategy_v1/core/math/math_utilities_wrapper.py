...
class MathUtilities:
    ...
    def __init__(self, config: Dict[str, Any], execution_mode: str, utility_manager=None):
        ...
        logger.info(f"MathUtilities initialized in {execution_mode} mode with UtilityManager: {utility_manager is not None}")
    ...
    def calculate_projected_ltv_after_borrowing(self, current_collateral_value: Decimal,
                                              current_debt_value: Decimal,
                                              additional_borrowing_usd: Decimal,
                                              collateral_efficiency: Decimal = Decimal('0.95')) -> Decimal:
        ...
        try:
            result = self.ltv_calculator.calculate_projected_ltv_after_borrowing(
                current_collateral_value, current_debt_value, 
                additional_borrowing_usd, collateral_efficiency
            )
    ...
    def calculate_margin_capacity(self, available_balance: Decimal, margin_requirement: Decimal,
                                 safety_buffer: Decimal = Decimal('0.1')) -> Decimal:
    ...
    def calculate_health_factor(self, collateral_value: Decimal, debt_value: Decimal,
                               liquidation_threshold: Decimal) -> Decimal:
    ...
    def calculate_metrics(self, portfolio, initial_capital: Decimal, timestamp: pd.Timestamp) -> Dict[str, Any]:
        """Calculate portfolio metrics with event logging."""
        try:
            result = self.metrics_calculator.calculate_metrics(portfolio, initial_capital, timestamp)
    ...
    def check_component_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'calculation_count': self.calculation_count,
            'last_calculation_timestamp': self.last_calculation_timestamp.isoformat() if self.last_calculation_timestamp else None,
    ...
    def get_market_price(self, token: str, currency: str, timestamp: pd.Timestamp) -> float:
        ...
        if self.utility_manager:
            ...
            return self.utility_manager.get_price_for_position_key(position_key, timestamp)
        else:
    ...
    def convert_to_usdt(self, amount: float, token: str, timestamp: pd.Timestamp) -> float:
        ...
        if self.utility_manager:
            position_key = f"wallet:BaseToken:{token}"
            return self.utility_manager.convert_position_to_usd(position_key, amount, timestamp)
        else:
