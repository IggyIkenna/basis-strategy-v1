...
class UnifiedHealthManager:
    ...
    def check_component_health(self, component_name: str) -> ComponentHealth:
        ...
        try:
            ...
            logger.debug(
                f"Health check for {component_name}: {status.value} ({check_duration:.3f}s)"
            )
        ...
        while self._monitoring_active:
            try:
                ...
                if system_health["system_status"] == "unhealthy":
                    logger.warning(
                        f"System health degraded: {system_health['unhealthy_components']} unhealthy components"
                    )
                elif system_health["system_status"] == "degraded":
                    logger.info(
                        f"System health degraded: {system_health['degraded_components']} degraded components"
                    )
    ...
    def report_error(self, component_name: str, error: ComponentError):
        ...
        with self._lock:
            if component_name in self._component_health:
                ...
                logger.warning(
                    f"Error reported from {component_name}: {error.error_code} - {error.message}"
                )
