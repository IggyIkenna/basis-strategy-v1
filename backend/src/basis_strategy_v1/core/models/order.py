...
class Order(BaseModel):
    ...
    operation_id: str = Field(
        ..., description="Unique operation identifier (generated by strategy)"
    )
    ...
    order_type: Literal["market", "limit"] = Field(
        "market", description="Order type (market or limit)"
    )
    ...
    # Venue and token flow (REQUIRED for all operations)
    source_venue: str = Field(..., description="Source venue where tokens/assets come from")
    target_venue: str = Field(..., description="Target venue where tokens/assets go to")
    source_token: str = Field(..., description="Token being sent/sold")
    target_token: str = Field(..., description="Token being received/bought")
    token: Optional[str] = Field(
        None, description="Token symbol (legacy, use source_token/target_token)"
    )

    # Expected position deltas (calculated by strategy BEFORE creating order)
    expected_deltas: Dict[str, float] = Field(
        ..., description="Expected position changes: instrument_key -> delta_amount"
    )

    # Operation-specific details
    operation_details: Dict[str, Any] = Field(
        default_factory=dict, description="Operation-specific parameters and context"
    )
    ...
    execution_mode: Literal["sequential", "atomic"] = Field(
        "sequential", description="Sequential or atomic execution"
    )
    ...
    strategy_intent: Optional[str] = Field(
        None, description="Human-readable strategy intent (entry_full, exit_partial, etc)"
    )
    strategy_id: Optional[str] = Field(None, description="Strategy identifier")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata for the order"
    )
    ...
    def validate_operation_requirements(self):
        ...
        # Transfer validation (source_venue and target_venue are now always required)
        if op == OrderOperation.TRANSFER:
            if not self.token and not (self.source_token and self.target_token):
                raise ValueError(f"{op} requires 'token' or 'source_token/target_token' parameters")
    ...
    model_config = {
        ...
        "extra": "allow",  # Allow additional fields for flexibility (operation_details covers this)
    }
