...
class OnChainExecutionInterface(BaseExecutionInterface):
    ...
    def _init_web3_clients(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized {len(self.web3_clients)} Web3 clients: {list(self.web3_clients.keys())}"
            )
    ...
    def _init_contracts(self):
        ...
        self.contracts = {"aave_v3": None, "uniswap_v3": None, "lido": None, "etherfi": None}
    ...
    async def _execute_backtest_operation(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if operation == "AAVE_SUPPLY":
            ...
            if token_out == "aUSDT":
                ...
                if market_data and "timestamp" in market_data:
                    liquidity_index = utility_manager.get_liquidity_index(
                        "USDT", market_data.get("timestamp")
                    )
                else:
                    # Use default liquidity index if market_data is not available
                    logger.warning(
                        "Market data not available for liquidity index lookup, using default value"
                    )
                    liquidity_index = 1.0  # Default fallback

                amount_out = amount / liquidity_index
                logger.info(
                    f"Onchain Execution: AAVE_SUPPLY USDT->aUSDT: {amount} USDT / {liquidity_index:.6f} = {amount_out:.2f} aUSDT"
                )
                onchain_interface_logger.info(
                    f"Onchain Execution: AAVE_SUPPLY USDT->aUSDT: {amount} USDT / {liquidity_index:.6f} = {amount_out:.2f} aUSDT"
                )
            else:
                ...
                amount_out = amount
                logger.info(
                    f"Onchain Execution: AAVE_SUPPLY {token_in}->{token_out}: {amount} -> {amount_out} (1:1 ratio)"
                )
                onchain_interface_logger.info(
                    f"Onchain Execution: AAVE_SUPPLY {token_in}->{token_out}: {amount} -> {amount_out} (1:1 ratio)"
                )
            ...
            await self._update_position_monitor(
                {"operation": operation, "token": token_in, "amount": amount, "gas_cost": gas_cost}
            )
    ...
    async def _execute_live_operation(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _build_transaction(
        ...
        return {"to": "0x" + "0" * 40, "value": 0, "gas": 21000, "data": b""}  # Placeholder address
    ...
    async def _get_backtest_position(
    ...
    async def execute_transfer(
        ...
        if self.execution_mode == "backtest":
            return await self._execute_backtest_transfer(
                operation, token, amount, venue, market_data
            )
        else:
            return await self._execute_live_transfer(operation, token, amount, venue, market_data)

    async def _execute_backtest_transfer(
        self, operation: str, token: str, amount: float, venue: str, market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_live_transfer(
        ...
        try:
            ...
            # Build transaction based on operation
            tx = await self._build_transfer_transaction(
                operation, token, amount, venue, market_data
            )
    ...
    async def _execute_backtest_borrow(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_live_borrow(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_backtest_supply(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_live_supply(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_backtest_swap(
        self, instruction: Dict[str, Any], market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
