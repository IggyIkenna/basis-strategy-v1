...
class CEXPositionInterface(BasePositionInterface):
    ...
    def _get_credentials(self, venue: str) -> Dict[str, str]:
        ...
        if venue == "binance":
            return {
                "api_key": os.getenv(f"BASIS_{environment}__CEX__BINANCE_API_KEY"),
                "secret": os.getenv(f"BASIS_{environment}__CEX__BINANCE_API_SECRET"),
            }
        elif venue == "bybit":
            return {
                "api_key": os.getenv(f"BASIS_{environment}__CEX__BYBIT_API_KEY"),
                "secret": os.getenv(f"BASIS_{environment}__CEX__BYBIT_API_SECRET"),
            }
        elif venue == "okx":
            return {
                "api_key": os.getenv(f"BASIS_{environment}__CEX__OKX_API_KEY"),
                "secret": os.getenv(f"BASIS_{environment}__CEX__OKX_API_SECRET"),
                "passphrase": os.getenv(f"BASIS_{environment}__CEX__OKX_PASSPHRASE"),
            }
        else:
    ...
    def _get_simulated_positions(self, timestamp: pd.Timestamp) -> Dict[str, Any]:
        ...
        return {
            "venue": self.venue,
            "timestamp": timestamp,
            "spot_balances": {"USDT": 1000.0, "ETH": 1.0, "BTC": 0.1},
            "perp_positions": {"ETHUSDT": {"size": 0.0, "side": "long", "unrealized_pnl": 0.0}},
    ...
    def _get_simulated_balance(self, asset: str, timestamp: pd.Timestamp) -> float:
        ...
        simulated_balances = {"USDT": 1000.0, "ETH": 1.0, "BTC": 0.1}
