...
class VenueInterfaceFactory:
    ...
    def _handle_error(self, error: Exception, context: str = "") -> None:
        ...
        logger.error(
            f"Venue Interface Factory error {error_code}: {str(error)}",
            extra={
    ...
    def create_venue_interface(
        interface_type: str, execution_mode: str, config: Dict[str, Any], data_provider=None
    ...
    def create_all_venue_interfaces(
        execution_mode: str, config: Dict[str, Any], data_provider=None
    ...
    def set_venue_dependencies(
        interfaces: Dict[str, BaseExecutionInterface], position_monitor, event_logger, data_provider
    ):
        """
        ...
        if interfaces.get("transfer") and hasattr(
            interfaces["transfer"], "set_execution_interfaces"
        ):
            interfaces["transfer"].set_execution_interfaces(
                interfaces.get("cex"), interfaces.get("onchain")
            )
            logger.info("Connected transfer interface with CEX and OnChain interfaces")

    @staticmethod
    def create_venue_position_interface(
        venue: str, execution_mode: str, config: Dict[str, Any]
    ) -> BasePositionInterface:
        """
    ...
    @staticmethod
    def get_venue_position_interfaces(
        venues: List[str], execution_mode: str, config: Dict[str, Any]
    ) -> Dict[str, BasePositionInterface]:
        """
    ...
    def update_state(timestamp, trigger_source: str, **kwargs) -> None:
        ...
        logger.debug(
            f"VenueInterfaceFactory.update_state called at {timestamp} from {trigger_source}"
        )
