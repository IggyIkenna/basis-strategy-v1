...
class ExecutionManager:
    ...
    def __init__(
        ...
        self.execution_timeout = config.get('component_config', {}).get('execution_manager', {}).get('execution_timeout', 30)
        self.max_retries = config.get('component_config', {}).get('execution_manager', {}).get('max_retries', 3)
        
        self.logger.info(f"ExecutionManager initialized in {execution_mode} mode")
    
    def process_orders(self, timestamp: pd.Timestamp, orders: List[Order]) -> List[ExecutionHandshake]:
        """
        ...
        try:
            ...
            for atomic_group_id, group_orders in atomic_groups.items():
                if atomic_group_id:
                    ...
                    handshakes = self._process_atomic_group(timestamp, group_orders, atomic_group_id)
                else:
            ...
            self.logger.info(f"Processed {len(orders)} orders, generated {len(all_handshakes)} handshakes")
    ...
    def _process_single_order(self, timestamp: pd.Timestamp, order: Order) -> Optional[ExecutionHandshake]:
        ...
        try:
            ...
            if handshake:
                ...
                reconciliation_success = self._reconcile_with_retry(handshake, order)
    ...
    def _process_atomic_group(self, timestamp: pd.Timestamp, orders: List[Order], atomic_group_id: str) -> List[ExecutionHandshake]:
        """Process an atomic group of orders."""
        try:
            self.logger.info(f"Processing atomic group: {atomic_group_id} with {len(orders)} orders")
            ...
            self._log_atomic_group_execution(handshakes, orders, atomic_group_id, group_success, timestamp)
    ...
    def _reconcile_with_retry(self, handshake: ExecutionHandshake, order: Order) -> bool:
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    success = self.position_update_handler.reconcile_execution_deltas(
                        execution_deltas=structured_deltas,
    ...
    def _convert_deltas_to_structured_format(self, deltas: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    def _log_operation_execution(self, handshake: ExecutionHandshake, order: Order, timestamp: pd.Timestamp) -> None:
        """Log operation execution event."""
        try:
            # Convert deltas to structured format
            position_deltas = []
            for position_key, delta_amount in handshake.actual_deltas.items():
                position_deltas.append({
                    'position_key': position_key,
                    'delta_amount': delta_amount,
                    'source': order.operation.value,
    ...
    def _log_atomic_group_execution(
        ...
        try:
            ...
            execution_time_ms = sum(
                h.execution_details.get('execution_time_ms', 0) 
                for h in handshakes
            )
    ...
    def _log_execution_delta(self, handshake: ExecutionHandshake, order: Order, timestamp: pd.Timestamp) -> None:
        """Log execution delta event."""
        try:
            # Convert deltas to structured format
            deltas = []
    ...
    def _log_tight_loop_execution(
        ...
        try:
            ...
            event = TightLoopExecutionEvent(
                ...
                execution_status=handshake.status.value,
