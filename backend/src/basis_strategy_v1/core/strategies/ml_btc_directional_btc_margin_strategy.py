...
class MLBTCDirectionalBTCMarginStrategy(BaseStrategyManager):
    """
    ...
    def __init__(self, config: Dict[str, Any], data_provider, exposure_monitor, 
        ...
        self.share_class = config.get('share_class', 'BTC')  # Share class currency
        self.asset = config.get('asset', 'BTC')  # Primary asset
        
        # Define and validate instrument keys
        self.entry_instrument = f"{Venue.WALLET.value}:BaseToken:BTC"
        ...
        for instrument in required_instruments:
            if instrument not in self.available_instruments:
                raise ValueError(
                    ...
                    f"Add to configs/modes/{config.get('mode', 'ml_btc_directional_btc_margin')}.yaml"
                )
    ...
    def _get_ml_predictions(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            # Get ML predictions from canonical data structure
            data = self.data_provider.get_data(timestamp)
            ml_predictions = data.get('ml_data', {}).get('predictions', {})
    ...
    def _create_entry_full_orders(self, equity: float, signal: str) -> List[Order]:
        ...
        try:
            ...
            order = Order(
                ...
                target_token='BTC',
                expected_deltas={
                    self.perp_instrument: target_position['btc_perp_position'] if signal == 'long' else -target_position['btc_perp_position']
                },
    ...
    def _create_entry_partial_orders(self, equity_delta: float, signal: str) -> List[Order]:
        ...
        try:
            ...
            order = Order(
                ...
                target_token='BTC',
                expected_deltas={
                    self.perp_instrument: partial_position if signal == 'long' else -partial_position
                },
                execution_mode='sequential',
    ...
    def _create_exit_full_orders(self, equity: float) -> List[Order]:
        ...
        try:
            ...
            order = Order(
                ...
                target_token='BTC',
                expected_deltas={
                    self.perp_instrument: -btc_position  # Close position
                },
                execution_mode='sequential',
    ...
    def _create_exit_partial_orders(self, equity_delta: float) -> List[Order]:
        ...
        try:
            ...
            order = Order(
                ...
                target_token='BTC',
                expected_deltas={
                    self.perp_instrument: -partial_exit if btc_position > 0 else partial_exit  # Close partial position
                },
                execution_mode='sequential',
                ...
                if amount > 0 and token != 'BTC':  # BTC is the target asset
                    ...
                    orders.append(Order(
                        ...
                        expected_deltas={
                            ...
                            f"{Venue.WALLET.value}:BaseToken:BTC": amount  # Simplified 1:1
                        },
                        execution_mode='sequential',
