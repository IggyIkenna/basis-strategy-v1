...
class PureLendingUSDTStrategy(BaseStrategyManager):
    ...
    def __init__(self, config: Dict[str, Any], data_provider, exposure_monitor, 
        ...
        self.lending_instrument = f"{Venue.AAVE_V3.value}:aToken:aUSDT"
    ...
    def generate_orders(
        ...
        try:
            ...
            for venue in self.lending_venues:
                current_lending += current_positions.get(f'{venue}_usdt_supply', 0.0)
            ...
            for venue in self.lending_venues:
                ...
                if supply_amount > 0:
                    ...
                    expected_deltas = {
                        ...
                        self.lending_instrument: supply_amount   # Gain aUSDT (1:1 for now, will be updated with AAVE index)
                    }
                    ...
                    if hasattr(self, 'utility_manager') and self.utility_manager:
                        try:
                            if hasattr(self.utility_manager, 'get_aave_supply_index'):
                                supply_index = self.utility_manager.get_aave_supply_index(self.asset)
