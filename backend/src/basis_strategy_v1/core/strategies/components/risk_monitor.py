...
try:
    from ....core.error_codes.error_code_registry import error_code_registry, get_error_info
except ImportError:
...
class RiskMonitor:
    ...
    def __init__(self, config: Dict[str, Any], position_monitor=None, exposure_monitor=None, 
        ...
        # Get mode-specific parameters (needed in calculate_overall_risk)
        self.mode = self.config.get('mode')
        self.asset = self.config.get('asset', 'ETH')
        self.lst_type = self.config.get('lst_type')
    ...
    def enable_debug_logging(self):
        ...
        # Mode-specific parameters already set in __init__
        
        # Mode-specific risk parameters
        self.margin_ratio_target = self.config.get('margin_ratio_target', 1.0)
        self.max_stake_spread_move = self.config.get('max_stake_spread_move', 0.03)
        self.max_ltv = self.config.get('max_ltv', 0.85)
        self.liquidation_threshold = self.config.get('liquidation_threshold', 0.90)
        ...
        # Risk thresholds from config (fail-fast, no defaults!)
        # These will raise KeyError if missing, forcing proper config
        try:
            # Use calculated safe LTV instead of hardcoded target_ltv
            self.aave_safe_ltv = self.safe_parameters['aave_ltv']['target_ltv']
            
            # Use default values for missing config fields (temporary fix)
            ...
            self.margin_warning_threshold = self.config.get('venues', {}).get('margin_warning_pct', 0.20)
            self.margin_critical_threshold = self.config.get('venues', {}).get('margin_critical_pct', 0.15)
            self.margin_liquidation = 0.10  # Venue constant (all CEXs)
            
            self.delta_threshold_pct = self.config.get('strategy', {}).get('rebalance_threshold_pct', 0.05)
        except KeyError as e:
            self._log_structured_error('RISK-004', f'Config validation failed: missing required field {e}', {
                'missing_field': str(e),
                'config_keys': list(self.config.keys()) if self.config else [],
                'strategy_keys': list(self.config.get('strategy', {}).keys()) if self.config else [],
                'venues_keys': list(self.config.get('venues', {}).keys()) if self.config else []
            })
            raise RiskMonitorError('RISK-004', f'Config validation failed: missing required field {e}', missing_field=str(e))
    ...
    async def assess_risk(self, exposure_data: Dict[str, Any], market_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Wrapper method for risk assessment that calls calculate_overall_risk.
        
        Args:
            exposure_data: Current exposure data from ExposureMonitor
    ...
    async def calculate_aave_ltv_risk(self, exposure_data: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate AAVE LTV risk using integrated calculators."""
        try:
            ...
            # Determine risk level using dynamic target
            if ltv >= self.aave_ltv_critical:
                level = 'critical'
                message = f'AAVE LTV critical: {ltv:.2%} >= {self.aave_ltv_critical:.2%}'
            elif ltv >= dynamic_ltv_target:
                level = 'warning'
                message = f'AAVE LTV warning: {ltv:.2%} >= dynamic target {dynamic_ltv_target:.2%}'
            else:
                level = 'safe'
                message = f'AAVE LTV safe: {ltv:.2%} < dynamic target {dynamic_ltv_target:.2%}'
    ...
    async def calculate_cex_margin_risk(self, exposure_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate margin ratio based on gross vs net exposure
            # This is a simplified calculation - in practice, you'd get actual margin data from CEX APIs
            margin_ratio = (cex_net_exposure / cex_gross_exposure) if cex_gross_exposure > 0 else 0.0
    ...
    def calculate_dynamic_ltv_target(self, lst_type: str = 'weeth') -> Decimal:
        ...
        try:
            ...
            dynamic_ltv = max(dynamic_ltv, Decimal("0.1"))  # Minimum 10% LTV
    ...
    async def simulate_cex_liquidation(self, exposure_data: Dict[str, Any], venue: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for venue_name in venues_to_check:
                ...
                margin_ratio = total_margin / position_value
