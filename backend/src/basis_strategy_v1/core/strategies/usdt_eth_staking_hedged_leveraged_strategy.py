...
class USDTETHStakingHedgedLeveragedStrategy(BaseStrategyManager):
    ...
    def __init__(
        ...
        required_instruments = [
            f"{Venue.WALLET.value}:BaseToken:USDT",
            f"{Venue.WALLET.value}:BaseToken:ETH",
            f"{Venue.ETHERFI.value}:LST:weETH",
            f"{Venue.AAVE_V3.value}:aToken:aWETH",
            f"{Venue.AAVE_V3.value}:debtToken:debtWETH",
            f"{Venue.INSTADAPP.value}:BaseToken:WETH",
            f"{Venue.BINANCE.value}:BaseToken:USDT",
            f"{Venue.BINANCE.value}:Perp:ETHUSDT",
            f"{Venue.BYBIT.value}:BaseToken:USDT",
            f"{Venue.BYBIT.value}:Perp:ETHUSDT",
            f"{Venue.OKX.value}:BaseToken:USDT",
            f"{Venue.OKX.value}:Perp:ETHUSDT",
        ]
        ...
        for instrument in required_instruments:
            if instrument not in self.available_instruments:
                raise ValueError(
                    ...
                    f"Add to configs/modes/{config.get('mode', 'usdt_eth_staking_hedged_leveraged')}.yaml"
                )

        # USDT ETH staking hedged leveraged-specific configuration (fail-fast access)
        self.stake_allocation_percentage = config[
            "stake_allocation_percentage"
        ]  # 50% for staking, 50% for hedging
        ...
        self.usdt_allocation = 1.0 - self.stake_allocation_percentage  # Remaining for USDT lending
        self.eth_allocation = self.stake_allocation_percentage  # For ETH staking

        logger.info(
            f"USDTETHStakingHedgedLeveragedStrategy initialized with {self.stake_allocation_percentage*100}% ETH staking, {self.usdt_allocation*100}% USDT lending, {self.leverage_multiplier}x leverage"
        )
    ...
    def generate_orders(
        ...
        try:
            ...
            has_position = any(
                current_positions.get("aUSDT_balance", 0.0) > 0
                or current_positions.get(f"{self.lst_type.lower()}_balance", 0.0) > 0
                for _ in [1]
            )
    ...
    def _create_entry_partial_orders(self, equity_delta: float) -> List[Order]:
        ...
        try:
            ...
            orders = []
            atomic_group_id = f"usdt_eth_staking_hedged_leveraged_partial_{int(equity_delta)}"
    ...
    def _create_exit_full_orders(self, equity: float) -> List[Order]:
        ...
        try:
            ...
            orders = []
            atomic_group_id = f"usdt_eth_staking_hedged_leveraged_exit_{int(equity)}"
            ...
            orders.append(
                Order(
                    ...
                    token=self.share_class,
                    amount=equity,
                    expected_deltas={f"wallet:BaseToken:{self.share_class}": equity},
                    execution_mode="sequential",
                    strategy_intent="exit_full",
                    strategy_id="usdt_eth_staking_hedged_leveraged",
                )
    ...
    def _create_exit_partial_orders(self, equity_delta: float) -> List[Order]:
        ...
        try:
            ...
            orders = []
            atomic_group_id = f"usdt_eth_staking_hedged_leveraged_partial_exit_{int(equity_delta)}"
            ...
            orders.append(
                Order(
                    ...
                    token=self.share_class,
                    amount=equity_delta,
                    expected_deltas={f"wallet:BaseToken:{self.share_class}": equity_delta},
                    execution_mode="sequential",
                    strategy_intent="exit_partial",
                    strategy_id="usdt_eth_staking_hedged_leveraged",
                )
            ...
            for token, amount in dust_tokens.items():
                if amount > 0 and token != self.share_class:
                    ...
                    if token == "USDT":
                        # Direct conversion
                        operation_id = (
                            f"transfer_dust_{token}_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        orders.append(
                            Order(
                                operation_id=operation_id,
                                ...
                                token=token,
                                amount=amount,
                                expected_deltas={f"wallet:BaseToken:{token}": amount},
                                execution_mode="sequential",
                        ...
                        operation_id = (
                            f"sell_dust_{token}_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        eth_price = self._get_asset_price()
                        orders.append(
                            Order(
                        ...
                        operation_id = (
                            f"unstake_dust_{token}_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        orders.append(
                            Order(
                                operation_id=operation_id,
                        ...
                        operation_id2 = (
                            f"sell_dust_eth_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        eth_price = self._get_asset_price()
                        orders.append(
                            Order(
                        ...
                        operation_id = (
                            f"withdraw_dust_{token}_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        orders.append(
                            Order(
                                operation_id=operation_id,
                        ...
                        operation_id2 = (
                            f"transfer_dust_usdt_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        orders.append(
                            Order(
                                operation_id=operation_id2,
                                ...
                                token="USDT",
                                amount=amount,
                                expected_deltas={"wallet:BaseToken:USDT": amount},
                                execution_mode="atomic",
                                atomic_group_id=atomic_group_id,
                                sequence_in_group=2,
                                strategy_intent="sell_dust",
                        ...
                        operation_id = (
                            f"swap_dust_{token}_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                        )
                        orders.append(
                            Order(
                                operation_id=operation_id,
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            base_info.update(
                {
                    "strategy_type": "usdt_eth_staking_hedged_leveraged",
