...
class ETHStakingOnlyStrategy(BaseStrategyManager):
    ...
    def __init__(
        ...
        # For staking only, allocate 100% to ETH staking
        ...
        logger.info(
            f"ETHStakingOnlyStrategy initialized with {self.eth_allocation*100}% ETH allocation, {self.lst_type}"
        )
    ...
    def _get_asset_price(self, timestamp: pd.Timestamp) -> float:
        """Get current ETH price using utility manager."""
        try:
            if self.utility_manager:
                return self.utility_manager.get_price_for_instrument_key(
                    "binance:BaseToken:ETH", timestamp, "USD"
                )
            else:
                # Fallback to mock price if utility manager not available
                return 3000.0
        except Exception as e:
            logger.warning(f"Failed to get ETH price, using fallback: {e}")
            return 3000.0

    def calculate_target_position(
        self, current_equity: float, timestamp: pd.Timestamp = None
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            eth_price = self._get_asset_price(timestamp)
            eth_amount = eth_target / eth_price if eth_price > 0 else 0

            return {
                "wallet:BaseToken:ETH": 0.0,  # No raw ETH, all staked
                f"etherfi:BaseToken:{self.lst_type.upper()}": eth_amount,  # All ETH converted to LST
                f"wallet:BaseToken:{self.share_class}": current_equity,
            ...
            return {
                "wallet:BaseToken:ETH": 0.0,
                f"etherfi:BaseToken:{self.lst_type.upper()}": 0.0,
                f"wallet:BaseToken:{self.share_class}": 0.0,  # No reserve balance needed
                "total_equity": current_equity,
            }
