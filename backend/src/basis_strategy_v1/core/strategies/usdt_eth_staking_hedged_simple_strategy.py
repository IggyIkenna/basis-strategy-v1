...
class USDTETHStakingHedgedSimpleStrategy(BaseStrategyManager):
    ...
    def __init__(
        ...
        for instrument in required_instruments:
            if instrument not in self.available_instruments:
                raise ValueError(
                    ...
                    f"Add to configs/modes/{config.get('mode', 'usdt_eth_staking_hedged_simple')}.yaml"
                )
        ...
        logger.info(
            f"USDTETHStakingHedgedSimpleStrategy initialized with {self.stake_allocation_percentage*100}% ETH staking, {self.usdt_allocation*100}% USDT lending"
        )
    ...
    def _create_exit_partial_orders(self, equity_delta: float) -> List[Order]:
        ...
        try:
            ...
            if lst_balance > 0:
                ...
                # Unstake partial ETH
                operation_id = (
                    f"unstake_eth_partial_{int(pd.Timestamp.now().timestamp() * 1000000)}"
                )
                orders.append(
                    Order(
                        operation_id=operation_id,
