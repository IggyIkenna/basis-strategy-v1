...
class PositionMonitor:
    ...
    def __init__(
        ...
        # Core position state (instrument_key -> amount)
    ...
    def _initialize_positions_from_config(self) -> None:
        ...
        for instrument_key in position_subs:
            validate_instrument_key(instrument_key)

        self.logger.info(f"Pre-initializing {len(position_subs)} validated positions")

        for instrument_key in position_subs:
            self.simulated_positions[instrument_key] = 0.0
            self.real_positions[instrument_key] = 0.0

        self.logger.info(f"Position universe: {list(self.simulated_positions.keys())}")
    ...
    def _initialize_position_interfaces(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Position interfaces initialized for {len(venues)} venues: {venues}")
    ...
    def _get_enabled_venues(self) -> List[str]:
        ...
        for instrument_key in self.simulated_positions.keys():
    ...
    def get_current_positions(self) -> Dict[str, float]:
        ...
        Returns:
            Dict mapping instrument_key -> amount
    ...
    def update_state(
        ...
        Args:
            ...
            trigger_source: Either 'execution_manager' (tight loop) or 'position_refresh' (timestep start)
        ...
        try:
            ...
            if self.last_timestamp != timestamp:
                ...
                self.logger.debug(f"New timestamp {timestamp}, reset settlement tracking")
            ...
            if trigger_source == "execution_manager":
                ...
                raise ValueError(
                    ...
                    f"Must be 'execution_manager' or 'position_refresh'."
                )
    ...
    def _apply_execution_deltas(
        self, timestamp: pd.Timestamp, execution_deltas: Union[Dict, List[Dict]]
    ) -> None:
        """
        Apply execution deltas from execution_manager (trades/transfers).
        Normalizes format and delegates to unified applier.

        Args:
            timestamp: Current timestamp
            execution_deltas: Single delta or list of deltas from execution_manager
        """
        # Normalize to list
    ...
    def _apply_position_deltas(self, timestamp: pd.Timestamp, deltas: List[Dict]) -> None:
        """
        Unified delta applier - handles ALL position updates.

        Used by:
        - _apply_execution_deltas() for trade/transfer deltas from execution_manager
        ...
        Args:
            ...
            deltas: List of delta dicts with format:
                {
                    "instrument_key": str,      # "venue:position_type:symbol"
        ...
        for delta in deltas:
            ...
            if instrument_key not in self.simulated_positions:
                raise ValueError(
                    f"Position '{instrument_key}' not found in position_subscriptions. "
                    f"All positions must be pre-declared in config. "
                    f"Declared positions: {list(self.simulated_positions.keys())}"
                )

            # Apply delta
            current_amount = self.simulated_positions[instrument_key]
            new_amount = current_amount + delta_amount
            self.simulated_positions[instrument_key] = new_amount
            
            # Log delta application
            self.logger.debug(
                f"Applied delta: {instrument_key} {delta_amount:+.6f} "
            ...
            self.logger.info(
                f"Position delta applied: {instrument_key}",
    ...
    def _query_venue_balances(self, timestamp: pd.Timestamp) -> None:
        ...
        if self.execution_mode == "backtest":
            ...
            if "INITIAL_CAPITAL" not in self.applied_this_timestamp and not any(
                v != 0 for v in self.simulated_positions.values()
            ):
                ...
                self.logger.info(f"Applied initial capital at {timestamp}")
                import time
                ...
                self.logger.info(
                    ...
                    instrument_key=initial_capital_deltas[0]["instrument_key"]
                    if initial_capital_deltas
                    else None,
                    timestamp=timestamp.isoformat(),
                    duration_ms=duration_ms,
                )
            ...
            if (
                self._should_apply_funding_settlement(timestamp)
                and "funding_settlement" not in self.applied_this_timestamp
            ):
                ...
                self.logger.info(f"Applied funding settlement at {timestamp}")

            # 3. APY growth for aToken positions (every hour)
            if (
                self._should_apply_apy_growth(timestamp)
                and "apy_growth" not in self.applied_this_timestamp
            ):
                automatic_deltas.extend(self._generate_apy_growth_deltas(timestamp))
                self.applied_this_timestamp.add("apy_growth")
                self.logger.info(f"Applied APY growth at {timestamp}")

            # 4. Margin PnL (if configured)
            if (
                self._should_apply_margin_pnl(timestamp)
                and "MARGIN_PNL" not in self.applied_this_timestamp
            ):
                automatic_deltas.extend(self._generate_margin_pnl_deltas(timestamp))
                self.applied_this_timestamp.add("MARGIN_PNL")
                self.logger.info(f"Applied margin PnL at {timestamp}")

            # 5. Seasonal rewards (daily if configured)
            if (
                self._should_apply_seasonal_rewards(timestamp)
                and "seasonal_rewards" not in self.applied_this_timestamp
            ):
                automatic_deltas.extend(self._generate_seasonal_rewards_deltas(timestamp))
                self.applied_this_timestamp.add("seasonal_rewards")
                self.logger.info(f"Applied seasonal rewards at {timestamp}")
    ...
    def _query_real_venue_positions(self, timestamp: pd.Timestamp) -> Dict[str, float]:
        ...
        Returns:
            Dict mapping instrument_key -> actual amount
        """
        queried_positions = {}
        ...
        for instrument_key, amount in queried_positions.items():
            if instrument_key in result:
                result[instrument_key] = amount
            else:
                # Position found but not declared in config
                self.logger.warning(
                    f"Venue returned undeclared position '{instrument_key}' with amount {amount}. "
    ...
    def _generate_initial_capital_deltas(self) -> List[Dict]:
        ...
        if self.share_class == "USDT":
            ...
            instrument_key = "wallet:BaseToken:ETH"
        else:
        ...
        if instrument_key not in self.simulated_positions:
            raise ValueError(
                f"Initial capital position '{instrument_key}' not found in position_subscriptions. "
                f"Add '{instrument_key}' to position_subscriptions in your mode YAML config."
            )
    ...
    def _generate_funding_settlement_deltas(self, timestamp: pd.Timestamp) -> List[Dict]:
        ...
        for instrument_key, position_size in perp_positions.items():
            if position_size == 0:
                continue

            # Calculate funding payment using utility_manager
            funding_payment = self.utility_manager.calculate_funding_payment(
                instrument_key=instrument_key, position_size=position_size, timestamp=timestamp
            )

            if funding_payment != 0:
                # Funding settles to USDT balance
                # TODO: Extract venue-specific USDT position from instrument_key
                usdt_instrument_key = self._get_usdt_position_for_venue(instrument_key)

                deltas.append(
                    {
                        "instrument_key": usdt_instrument_key,
                        "delta_amount": funding_payment,
                        "source": "funding_settlement",
                        "price": None,
                        "fee": None,
                        "metadata": {
                            "perp_position": instrument_key,
    ...
    def _generate_seasonal_rewards_deltas(self, timestamp: pd.Timestamp) -> List[Dict]:
        ...
        staking_positions = {
            k: v
            for k, v in self.simulated_positions.items()
            if any(token in k for token in ["stETH", "eETH", "wstETH", "weETH"])
        }

        for instrument_key, position_size in staking_positions.items():
            if position_size == 0:
                continue
    ...
    def _should_apply_apy_growth(self, timestamp: pd.Timestamp) -> bool:
        """Check if APY growth should be applied (every hour)."""
        # Apply APY growth every hour
        return timestamp.minute == 0

    def _generate_apy_growth_deltas(self, timestamp: pd.Timestamp) -> List[Dict]:
        """
        Generate APY growth deltas for aToken positions.
        
        In AAVE, aToken quantity stays the same but value grows via supply index.
        This simulates the lending yield accumulation.
        """
        deltas = []
        
        # Calculate APY growth (simplified - 5% APY, hourly rate)
        hourly_rate = 0.05 / (365 * 24)  # 5% APY / 365 days / 24 hours
        
        # Apply growth to all aToken positions
        for instrument_key, amount in self.simulated_positions.items():
            if ":aToken:" in instrument_key and amount > 0:
                # Calculate growth amount (this represents the value increase)
                growth_amount = amount * hourly_rate
                
                # For aToken positions, we don't change the quantity
                # The growth is reflected in the value calculation in ExposureMonitor
                # But we can log it for tracking
                self.logger.debug(
                    f"APY growth for {instrument_key}: {growth_amount:.6f} "
                    f"(rate: {hourly_rate:.6f}, amount: {amount:.6f})"
                )
        
        return deltas

    def _get_usdt_position_for_venue(self, perp_instrument_key: str) -> str:
        """
        ...
        """
        # Format: venue:position_type:symbol
        parts = perp_instrument_key.split(":")
        venue = parts[0]
