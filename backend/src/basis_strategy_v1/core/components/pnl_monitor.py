...
from ...core.math.equity_calculator import calculate_equity
...
class PnLMonitorError(Exception):
...
class PnLMonitor:
    ...
    def __init__(
        ...
        Args:
            ...
            position_monitor: Position monitor reference for getting current positions
        """
        self.config = config
        ...
        # Initialize previous positions for P&L calculation
    ...
    def _initialize_pnl_tracking(self):
        ...
        currency_symbol = (
            ...
            if self.share_class == "BTC"
            else "$"
        )

        logger.info(
            f"P&L Calculator initialized for {self.share_class} share class with {self.initial_capital:,.2f} {currency_symbol} initial capital"
        )
        self.logger.info(
    ...
    def _calculate_pnl(
        ...
        """
        Private method to calculate P&L and store results.
        ...
        For triggering calculations, use update_state().

        Triggered by: update_state() method calls
        """
        try:
            ...
            # Get current positions from position_monitor
            if not self.position_monitor:
                raise PnLMonitorError(
                    "PNL-003",
                    message="Position monitor required for P&L calculation",
                )

            current_positions = self.position_monitor.get_current_positions()
            self.logger.info(f"P&L Calculator: Retrieved {len(current_positions)} positions from position monitor")

            # Set initial value if first calculation
            if self.initial_total_value is None:
                # Use initial capital as baseline
                self.initial_total_value = self.initial_capital
                logger.info(
                    f"Initial portfolio value set to ${self.initial_total_value:,.2f} (initial capital)"
                )
                self.logger.info(
                    f"P&L Calculator: Initial portfolio value set to ${self.initial_total_value:,.2f} (initial capital)"
                )
            else:
                self.logger.info(
                    f"P&L Calculator: Using existing initial_total_value: ${self.initial_total_value:,.2f}"
                )
            ...
            if self.utility_manager:
                ...
                share_class = self.utility_manager.get_share_class_from_mode(
                    self.config.get("mode", "default")
                )
                self.logger.info(f"P&L Calculator: Using share class {share_class} from config")

            # 1. Balance-Based P&L (source of truth) - now using equity calculation
            self.logger.info(f"P&L Calculator: About to calculate balance-based P&L using equity calculator")
            balance_pnl_data = self._calculate_balance_based_pnl(
                current_positions, period_start, current_time=timestamp
            )
            self.logger.info(f"P&L Calculator: Balance-based P&L calculated successfully")

            # 2. Attribution P&L (breakdown) - simplified for now
            ...
            reconciliation = self._reconcile_pnl(
                balance_pnl_data, attribution_pnl_data, period_start, current_time=timestamp
            )
            ...
            # Store previous positions for next calculation
            ...
            logger.debug(
                f"P&L calculated: balance=${balance_pnl_data['PNL_CUMULATIVE']:,.2f}, attribution=${attribution_pnl_data['PNL_CUMULATIVE']:,.2f}"
            )
            return pnl_data

        except PnLMonitorError:
    ...
    def _calculate_balance_based_pnl(
        self, current_positions: Dict[str, float], period_start: pd.Timestamp, current_time: pd.Timestamp
    ) -> Dict:
        """Calculate P&L from portfolio equity change in share class currency."""
        try:
            self.logger.info(
                f"P&L Calculator: _calculate_balance_based_pnl called with {len(current_positions)} positions"
            )

            if not self.utility_manager:
                raise PnLMonitorError(
                    "PNL-003",
                    message="Utility manager required for equity calculation",
                )

            # Calculate current equity using equity calculator
            ...
            current_value = equity_data['total_equity']

            # Calculate P&L as the change in equity since initial snapshot
            # This is mode-agnostic - it measures the change in total portfolio equity in share class currency
            pnl_cumulative = current_value - self.initial_total_value
            self.logger.info(
                f"P&L Calculator: Balance-based P&L - current_equity: ${current_value:,.2f}, initial_total_value: ${self.initial_total_value:,.2f}, pnl_cumulative: ${pnl_cumulative:,.2f}"
            )

            # Calculate hourly P&L if we have previous positions
            if self.previous_positions:
                previous_equity_data = calculate_equity(
                    positions=self.previous_positions,
                    utility_manager=self.utility_manager,
                    share_class=self.share_class,
                    timestamp=period_start
                )
                previous_value = previous_equity_data['total_equity']
    ...
    def _calculate_attribution_pnl(
        self, current_exposure: Dict, previous_exposure: Optional[Dict], timestamp: pd.Timestamp
    ) -> Dict:
        ...
        if not isinstance(previous_exposure, dict):
            self.logger.warning(
                f"Previous exposure is not a dict: {type(previous_exposure)}, returning zero attribution"
            )
            return self._zero_attribution()
        ...
        return self._calculate_config_driven_attribution(
            current_exposure, previous_exposure, timestamp
        )

    def _calculate_config_driven_attribution(
        self, current_exposure: Dict, previous_exposure: Dict, timestamp: pd.Timestamp
    ) -> Dict:
        ...
        for attr_type in self.attribution_types:
            try:
                if attr_type == "supply_yield":
                    attribution_pnl[attr_type] = self._calc_supply_yield(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "borrow_costs":
                    attribution_pnl[attr_type] = self._calc_borrow_costs(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "staking_yield_oracle":
                    attribution_pnl[attr_type] = self._calc_staking_yield_oracle(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "staking_yield_rewards":
                    attribution_pnl[attr_type] = self._calc_staking_yield_rewards(
                        current_exposure, previous_exposure
                    )
                    ...
                    attribution_pnl[attr_type] = self._calc_delta_pnl(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "basis_pnl":
                    attribution_pnl[attr_type] = self._calc_basis_pnl(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "dust_pnl":
                    attribution_pnl[attr_type] = self._calc_dust_pnl(
                        current_exposure, previous_exposure
                    )
                elif attr_type == "transaction_costs":
                    attribution_pnl[attr_type] = self._calc_transaction_costs(
                        current_exposure, previous_exposure
                    )
                else:
    ...
    def _calc_supply_yield(self, current: Dict, previous: Dict) -> float:
        ...
        if not isinstance(current, dict) or not isinstance(previous, dict):
            self.logger.warning(
                f"Supply yield calculation: current type: {type(current)}, previous type: {type(previous)}"
            )
            return 0.0
        ...
        if current_aweeth and previous_aweeth:
            ...
            if previous_underlying > 0:
                ...
                if self.utility_manager:
                    oracle_price = self.utility_manager.get_oracle_price(
                        "weETH", current_aweeth.get("timestamp")
                    )
                    eth_price = self.utility_manager.get_market_price(
                        "ETH", current_aweeth.get("timestamp")
                    )
                else:
        ...
        if current_ausdt and previous_ausdt:
            ...
            current_underlying = current_ausdt.get(
                "underlying_balance", current_ausdt.get("underlying_native", 0)
            )
            previous_underlying = previous_ausdt.get(
                "underlying_balance", previous_ausdt.get("underlying_native", 0)
            )
    ...
    def _calc_staking_yield_oracle(self, current: Dict, previous: Dict) -> float:
        ...
        if not isinstance(current, dict) or not isinstance(previous, dict):
            self.logger.warning(
                f"Staking yield oracle calculation: current type: {type(current)}, previous type: {type(previous)}"
            )
            return 0.0
        ...
        if self.utility_manager:
            current_oracle = self.utility_manager.get_oracle_price(
                "weETH", current_aweeth.get("timestamp")
            )
            previous_oracle = self.utility_manager.get_oracle_price(
                "weETH", previous_aweeth.get("timestamp")
            )
            eth_price = self.utility_manager.get_market_price(
                "ETH", current_aweeth.get("timestamp")
            )
    ...
    def _calc_price_change_pnl(self, current: Dict, previous: Dict) -> float:
        ...
        if not isinstance(current, dict) or not isinstance(previous, dict):
            self.logger.warning(
                f"Price change PnL calculation: current type: {type(current)}, previous type: {type(previous)}"
            )
            return 0.0
        ...
        if self.utility_manager:
            price_change = self.utility_manager.convert_price(
                previous_eth_price, current_eth_price, 1.0
            )
        else:
    ...
    def _calc_borrow_costs(self, current: Dict, previous: Dict) -> float:
        ...
        if not isinstance(current, dict) or not isinstance(previous, dict):
            self.logger.warning(
                f"Borrow cost calculation: current type: {type(current)}, previous type: {type(previous)}"
            )
            return 0.0
        ...
        for token, exp in current["exposures"].items():
            if "PERP" in token and "UNREALIZED_PNL" in exp:
                ...
                if position_size > 0:
                    ...
                    if self.utility_manager:
                        funding_rate = self.utility_manager.get_funding_rate(
                            "binance", token, timestamp
                        )
                    else:
    ...
    def _get_btc_funding_rate(self, venue: str, timestamp: pd.Timestamp) -> float:
        ...
        if not self.utility_manager:
            raise PnLMonitorError(
                "PNL-BTC-001", f"No utility manager available for funding rate lookup for {venue}"
            )
        ...
        try:
            ...
            raise PnLMonitorError("PNL-BTC-001", f"Failed to get funding rate for {venue}: {e}")
    ...
    def _get_btc_hedged_exposure(self, exposure: Dict) -> float:
        ...
        total_perp = sum(
            abs(position.get("size", 0)) for position in cex_perps.values() if position
        )
    ...
    def _calc_delta_pnl(self, current: Dict, previous: Dict) -> float:
        ...
        if not isinstance(current, dict) or not isinstance(previous, dict):
            self.logger.warning(
                f"Delta PnL calculation: current type: {type(current)}, previous type: {type(previous)}"
            )
            return 0.0
        ...
        # Use utility manager for price conversions
        if self.utility_manager:
            current_eth_price = self.utility_manager.get_market_price(
                "ETH", current.get("timestamp")
            )
            previous_eth_price = self.utility_manager.get_market_price(
                "ETH", previous.get("timestamp")
            )
        else:
            current_eth_price = (
                current.get("exposures", {}).get("aWeETH", {}).get("eth_usd_price", 3000.0)
            )
            previous_eth_price = (
                previous.get("exposures", {}).get("aWeETH", {}).get("eth_usd_price", 3000.0)
            )
    ...
    def _reconcile_pnl(
        ...
        period_months = (current_time - period_start).total_seconds() / (
            30 * 24 * 3600
        )  # Approximate months
    ...
    def _get_pnl_summary(self, pnl_data: Dict) -> str:
        ...
        summary += (
            f"Difference: ${recon['difference']:,.2f} ({recon['DIFF_PCT_OF_CAPITAL']:.3f}%)\n"
        )
    ...
    def _calc_transaction_costs(self, current: Dict, previous: Dict) -> float:
        ...
        if previous:
            ...
            for venue in current_spot:
                ...
                previous_size = (
                    previous_spot.get(venue, {}).get("size", 0) if previous_spot.get(venue) else 0
                )
                ...
                if abs(current_size - previous_size) > 0.001:  # Position changed
                    ...
                    current_price = (
                        current_spot[venue].get("price", 50000) if current_spot[venue] else 50000
                    )
            ...
            for venue in current_perp:
                ...
                previous_size = (
                    previous_perp.get(venue, {}).get("size", 0) if previous_perp.get(venue) else 0
                )
                ...
                if abs(current_size - previous_size) > 0.001:  # Position changed
                    ...
                    traded_amount = abs(current_size - previous_size)
                    current_price = (
                        current_perp[venue].get("MARK_PRICE", 50000)
                        if current_perp[venue]
                        else 50000
                    )
                    transaction_costs += traded_amount * current_price * 0.001
                ...
                for lst_token in lst_tokens:
                    if lst_token in asset:
                        ...
                        if current_lst_amount > 0 or previous_lst_amount > 0:
                            ...
                            current_lst_price = self._get_aave_oracle_price(
                                lst_token, current.get("timestamp")
                            )
                            previous_lst_price = self._get_aave_oracle_price(
                                lst_token, previous.get("timestamp")
                            )
                            ...
                            if current_lst_price > 0 and previous_lst_price > 0:
                                ...
                                if self.utility_manager:
                                    price_change = self.utility_manager.convert_price(
                                        previous_lst_price, current_lst_price, 1.0
                                    )
                                else:
                                    price_change = current_lst_price - previous_lst_price

                                logger.debug(
                                    f"Staking yield for {lst_token}: amount_change={current_lst_amount - previous_lst_amount}, price_change={price_change:.6f}, yield={asset_yield:.2f}"
                                )
        ...
        try:
            ...
            for asset in current_exposures.keys():
                ...
                if current_spot_long > 0 and current_perp_short > 0:
                    ...
                    current_perp_price = current_asset_exposure.get("PERP_SHORT", {}).get(
                        "price", 0
                    )
                    previous_spot_price = previous_asset_exposure.get("SPOT_LONG", {}).get(
                        "price", 0
                    )
                    previous_perp_price = previous_asset_exposure.get("PERP_SHORT", {}).get(
                        "price", 0
                    )

                    if (
                        current_spot_price > 0
                        and current_perp_price > 0
                        and previous_spot_price > 0
                        and previous_perp_price > 0
                    ):
                        ...
                        logger.debug(
                            f"Basis P&L for {asset}: min_pos={min_position}, spread_change={spread_change:.6f}, pnl={asset_basis_pnl:.2f}"
                        )
                ...
                for dust_token in dust_tokens:
                    if dust_token in asset:
                        ...
                        if current_dust_amount > 0 or previous_dust_amount > 0:
                            ...
                            current_dust_price = self._get_protocol_token_price(
                                dust_token, current.get("timestamp")
                            )
                            previous_dust_price = self._get_protocol_token_price(
                                dust_token, previous.get("timestamp")
                            )
                            ...
                            if current_dust_price > 0 and previous_dust_price > 0:
                                ...
                                if self.utility_manager:
                                    price_change = self.utility_manager.convert_price(
                                        previous_dust_price, current_dust_price, 1.0
                                    )
                                else:
                                    price_change = current_dust_price - previous_dust_price

                                logger.debug(
                                    f"Dust P&L for {dust_token}: amount_change={current_dust_amount - previous_dust_amount}, price_change={price_change:.6f}, pnl={asset_dust_pnl:.2f}"
                                )
    ...
    def _get_protocol_token_price(self, token: str, timestamp: pd.Timestamp) -> float:
        ...
        try:
            ...
            data = self.utility_manager.data_provider.get_data(timestamp)
            protocol_prices = data["market_data"]["protocol_token_prices"]

            # Map token names to price keys
            token_mapping = {"EIGEN": "eigen", "ETHFI": "ethfi", "KING": "king"}
