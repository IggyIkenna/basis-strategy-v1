...
class RiskMonitor:
    ...
    def _initialize_risk_parameters(self):
        ...
        try:
            ...
            logger.debug(
                f"max_stake_spread_move_value: {max_stake_spread_move_value} (type: {type(max_stake_spread_move_value)})"
            )
            ...
            self.target_ltv = self.utility_manager.calculate_dynamic_ltv_target(
                self.aave_max_ltv_emode, max_stake_spread_move
            )
            ...
            for venue, requirements in self.cex_margin_requirements.items():
                self.cex_target_margins[venue] = self.utility_manager.calculate_cex_target_margin(
                    requirements["INITIAL_MARGIN"], max_underlying_move
                )

            # Load component-specific configuration with fail-fast behavior
            self.enabled_risk_types = self.config["component_config"]["risk_monitor"][
                "enabled_risk_types"
            ]
            ...
            logger.error(
                f"Config values - max_stake_spread_move: {self.config.get('max_stake_spread_move')}, max_underlying_move: {self.config.get('max_underlying_move')}"
            )
    ...
    def _load_risk_parameters(self):
        ...
        try:
            ...
            (
                ...
                self.aave_liquidation_threshold_emode,
            ) = self.risk_data_loader.get_aave_ltv_limits("emode", "weETH_WETH")
            self.aave_liquidation_bonus_emode = self.risk_data_loader.get_aave_liquidation_bonus(
                "emode", "weETH_WETH"
            )
            ...
            for venue in self.risk_data_loader.get_available_cex_venues():
                ...
                ) = self.risk_data_loader.get_cex_margin_requirements(venue)
            ...
            logger.info(
                f"Risk parameters loaded: AAVE (max_ltv: {self.aave_max_ltv_emode}), CEX venues: {list(self.cex_margin_requirements.keys())}"
            )
            ...
            self.cex_margin_requirements = {
                "binance": {
                    ...
                    "MAINTENANCE_MARGIN": Decimal("0.15"),
    ...
    def assess_risk(
        self, exposure_data: Dict, market_data: Dict, timestamp: pd.Timestamp
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_current_ltv(self, exposure_data: Dict) -> Decimal:
        ...
        try:
            ...
            for instrument_key, position_data in exposures.items():
                if "aave" in instrument_key.lower():
                    ...
                    if "aToken" in instrument_key:  # Collateral
                        total_collateral += value_usd
                    elif (
                        "debt" in instrument_key.lower() or "borrow" in instrument_key.lower()
                    ):  # Debt
            ...
            logger.debug(
                f"Current LTV calculation: debt={total_debt}, collateral={total_collateral}, ltv={current_ltv}"
            )
    ...
    def _calculate_cex_margin_ratios(self, exposure_data: Dict) -> Dict[str, Decimal]:
        ...
        try:
            ...
            venue_positions = {}
            for instrument_key, position_data in exposures.items():
                venue = instrument_key.split(":")[0] if ":" in instrument_key else "unknown"
                if venue in ["binance", "bybit", "okx"]:
                    ...
                    if "spot" in instrument_key.lower() or amount > 0:
    ...
    def _calculate_health_ratios(
        self, current_ltv: Decimal, cex_margin_ratios: Dict[str, Decimal]
    ) -> Dict[str, Decimal]:
        ...
        try:
            ...
            for venue, margin_ratio in cex_margin_ratios.items():
                if venue in self.cex_margin_requirements:
                    liquidation_threshold = self.cex_margin_requirements[venue][
                        "liquidation_threshold"
                    ]
                    if margin_ratio > 0:
                        ...
                        health_ratios[f"cex_{venue}"] = Decimal(
                            "999"
                        )  # No positions = infinite health
    ...
    def _calculate_maintenance_margin_ratios(
        self, cex_margin_ratios: Dict[str, Decimal]
    ) -> Dict[str, Decimal]:
    ...
    def _update_last_risks(self, overall_risk: float, timestamp: pd.Timestamp):
        """Update last risks."""
        try:
            self.last_risks = {"timestamp": timestamp, "overall_risk": overall_risk}
        except Exception as e:
    ...
    def get_current_risk_metrics(self) -> Dict[str, Any]:
        ...
        try:
            return {"LAST_RISKS": self.last_risks, "mode_agnostic": True}
        except Exception as e:
            logger.error(f"Error getting current risk metrics: {e}")
            return {"LAST_RISKS": None, "mode_agnostic": True, "error": str(e)}
