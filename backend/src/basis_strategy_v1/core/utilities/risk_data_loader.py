...
class RiskDataLoader:
    ...
    def load_aave_risk_parameters(self) -> Dict[str, Any]:
        ...
        try:
            risk_params_path = self.data_dir / 'protocol_data/aave/risk_params/aave_v3_risk_parameters.json'
    ...
    def load_cex_margin_requirements(self, venue: str) -> Dict[str, Any]:
        ...
        try:
            margin_params_path = self.data_dir / f'market_data/derivatives/risk_params/{venue}_margin_requirements.json'
            
            if not margin_params_path.exists():
                raise FileNotFoundError(f"CEX margin requirements file not found: {margin_params_path}")
    ...
    def get_aave_ltv_limits(self, mode: str = 'emode', collateral_pair: str = 'weETH_WETH') -> Tuple[Decimal, Decimal]:
        """
        ...
        try:
            ...
            if collateral_pair not in mode_params['ltv_limits']:
                raise ValueError(f"Collateral pair {collateral_pair} not found in {mode} LTV limits")
            
            max_ltv = Decimal(str(mode_params['ltv_limits'][collateral_pair]))
            liquidation_threshold = Decimal(str(mode_params['liquidation_thresholds'][collateral_pair]))
    ...
    def get_aave_liquidation_bonus(self, mode: str = 'emode', collateral_pair: str = 'weETH_WETH') -> Decimal:
        """
        ...
        try:
            ...
            if collateral_pair not in mode_params['liquidation_bonus']:
                raise ValueError(f"Collateral pair {collateral_pair} not found in {mode} liquidation bonus")
