"""
USDT Market Neutral Data Provider

Provides data for full USDT market neutral strategy (staking + leverage + hedging).
...
Reference: configs/modes/usdt_market_neutral.yaml
...
class USDTMarketNeutralDataProvider(BaseDataProvider):
    """Data provider for USDT market neutral strategy"""
    ...
    def validate_data_requirements(self, data_requirements: List[str]) -> None:
        ...
        if unsupported:
            raise ValueError(
                f"USDTMarketNeutralDataProvider cannot satisfy requirements: {unsupported}. "
                f"Available: {self.available_data_types}"
            )
        
        logger.info(f"✅ USDTMarketNeutralDataProvider can satisfy all requirements: {data_requirements}")
    
    def load_data(self) -> None:
        """Load all required data for USDT market neutral strategy."""
        logger.info(f"🔄 Loading data for USDT market neutral strategy...")
        ...
        try:
            ...
            logger.info(f"✅ USDT market neutral data loaded successfully: {len(self.data)} datasets")
            
        except Exception as e:
            logger.error(f"❌ Failed to load USDT market neutral data: {e}")
            raise DataProviderError('DATA-004', message=f"USDT market neutral data loading failed: {e}")
    
    def get_data(self, timestamp: pd.Timestamp) -> Dict[str, Any]:
        """
        Return standardized data structure for USDT market neutral strategy.
    ...
    def _validate_loaded_data(self) -> None:
        ...
        if missing_data:
            raise DataProviderError(
                'DATA-004',
                message=f"Missing required data for USDT market neutral: {missing_data}",
                missing_data=missing_data,
                mode=self.mode
            )
        
        logger.info("✅ All USDT market neutral data requirements satisfied")
