#!/usr/bin/env python3
"""
Fix all Order model tests by adding required fields.
"""

import re

def fix_all_order_tests():
    """Fix all Order model tests by adding required fields."""
    ...
    # Fix wallet transfer creation test
    content = re.sub(
        r'def test_wallet_transfer_creation\(self\):\s*"""Test creating a valid wallet transfer order\."""\s*order = Order\(\s*venue=\'wallet\',\s*operation=OrderOperation\.TRANSFER,\s*token=\'USDT\',\s*amount=5000\.0\s*\)',
    ...
    # Fix CEX trade validation missing side test
    content = re.sub(
        r'def test_cex_trade_validation_missing_side\(self\):\s*"""Test that CEX trades require side parameter\."""\s*with pytest\.raises\(ValidationError\) as exc_info:\s*Order\(\s*venue=\'binance\',\s*operation=OrderOperation\.SPOT_TRADE,\s*pair=\'BTC/USDT\',\s*amount=0\.5\s*\)\s*assert "requires \'side\' parameter" in str\(exc_info\.value\)',
    ...
    # Fix DeFi operation validation missing token_in test
    content = re.sub(
        r'def test_defi_operation_validation_missing_token_in\(self\):\s*"""Test that DeFi operations require token_in parameter\."""\s*with pytest\.raises\(ValidationError\) as exc_info:\s*Order\(\s*venue=\'aave\',\s*operation=OrderOperation\.SUPPLY,\s*token_out=\'aUSDT\',\s*amount=10000\.0\s*\)\s*assert "requires \'token_in\' parameter" in str\(exc_info\.value\)',
    ...
    print("Fixed all Order model tests!")
