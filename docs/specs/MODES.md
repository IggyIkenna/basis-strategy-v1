...
- Each strategy mode inherits from base strategy manager
...
**Note**: There are 7 YAML configs but 6 documented strategies. The missing strategy is **ETH Basis Trading Mode** which is documented below.
...
### Unified Interface
All strategy modes use the same unified interface for position changes:
- **Initial Setup**: Deploy capital according to strategy
- **Deposits**: Scale up existing positions
- **Withdrawals**: Scale down existing positions  
- **Rebalancing**: Adjust positions to maintain targets

### Mode-Specific Logic
The strategy manager contains mode-specific logic for:
...
2. **Generic Execution Manager**: Venue-based execution for all modes
3. **Unified Rebalancing**: Common rebalancing logic with mode-specific parameters
4. **Config-Driven Components**: Use configuration instead of mode-specific code
