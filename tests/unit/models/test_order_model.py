...
class TestOrderModel:
    ...
    def test_venue_type_detection(self):
        ...
        # CEX venues
        binance_order = Order(venue='binance', operation=OrderOperation.SPOT_TRADE, pair='BTC/USDT', side='BUY', amount=0.5)
        assert binance_order.get_venue_type() == VenueType.CEX
        
        bybit_order = Order(venue='bybit', operation=OrderOperation.PERP_TRADE, pair='BTCUSDT', side='SHORT', amount=1.0)
        assert bybit_order.get_venue_type() == VenueType.CEX
        
        # DeFi venues
        aave_order = Order(venue='aave', operation=OrderOperation.SUPPLY, token_in='USDT', amount=10000.0)
        assert aave_order.get_venue_type() == VenueType.DEFI
        
        etherfi_order = Order(venue='etherfi', operation=OrderOperation.STAKE, token_in='ETH', amount=2.0)
        assert etherfi_order.get_venue_type() == VenueType.DEFI
        
        # DeFi middleware
        instadapp_order = Order(venue='instadapp', operation=OrderOperation.FLASH_BORROW, token_out='WETH', amount=10.0)
        assert instadapp_order.get_venue_type() == VenueType.DEFI_MIDDLEWARE
        
        # Wallet
        wallet_order = Order(venue='wallet', operation=OrderOperation.TRANSFER, source_venue='wallet', target_venue='binance', token='USDT', amount=5000.0)
    ...
    def test_operation_type_checks(self):
        ...
        # CEX trade
        spot_order = Order(venue='binance', operation=OrderOperation.SPOT_TRADE, pair='BTC/USDT', side='BUY', amount=0.5)
        ...
        perp_order = Order(venue='bybit', operation=OrderOperation.PERP_TRADE, pair='BTCUSDT', side='SHORT', amount=1.0)
        assert perp_order.is_cex_trade() is True
        
        # DeFi operation
        supply_order = Order(venue='aave', operation=OrderOperation.SUPPLY, token_in='USDT', amount=10000.0)
        ...
        stake_order = Order(venue='etherfi', operation=OrderOperation.STAKE, token_in='ETH', amount=2.0)
        assert stake_order.is_defi_operation() is True
        
        # Wallet transfer
        transfer_order = Order(venue='wallet', operation=OrderOperation.TRANSFER, source_venue='wallet', target_venue='binance', token='USDT', amount=5000.0)
    ...
    def test_all_order_operations(self):
        ...
        for operation in operations:
            ...
            if operation in [OrderOperation.SPOT_TRADE, OrderOperation.PERP_TRADE]:
                order = Order(venue='binance', operation=operation, pair='BTC/USDT', side='BUY', amount=0.5)
            elif operation in [OrderOperation.SUPPLY, OrderOperation.STAKE, OrderOperation.SWAP]:
                order = Order(venue='aave', operation=operation, token_in='USDT', amount=1000.0)
            elif operation == OrderOperation.TRANSFER:
                order = Order(venue='wallet', operation=operation, source_venue='wallet', target_venue='binance', token='USDT', amount=1000.0)
            else:
                # For other operations (BORROW, REPAY, WITHDRAW, UNSTAKE, FLASH_BORROW, FLASH_REPAY), create minimal order
                order = Order(venue='aave', operation=operation, amount=1000.0)
