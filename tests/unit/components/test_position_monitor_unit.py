...
class TestPositionMonitorUnit:
    ...
    def test_collect_positions_structure(self, mock_config, mock_data_provider, mock_utility_manager):
        ...
        # Act
        snapshot = position_monitor.get_current_positions()
        
        # Assert
        assert isinstance(snapshot, dict)
        
        # Validate position keys structure (flat dictionary with position keys)
        for key, value in snapshot.items():
            assert isinstance(key, str)
            assert isinstance(value, (int, float))
            
        # Should have some position subscriptions from config
        expected_keys = [
    ...
    def test_state_persistence_across_timesteps(self, mock_config, mock_data_provider, mock_utility_manager):
        ...
        snapshot1 = position_monitor.get_current_positions()
        
        # Simulate time passing and get another snapshot
        snapshot2 = position_monitor.get_current_positions()
