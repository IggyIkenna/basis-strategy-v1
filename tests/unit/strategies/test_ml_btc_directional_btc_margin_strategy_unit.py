"""
Unit tests for ML BTC Directional (BTC Margin) Strategy.
...
from backend.src.basis_strategy_v1.core.strategies.ml_btc_directional_btc_margin_strategy import (
...
class TestMLBTCDirectionalBTCMarginStrategyInit:
    ...
    def test_init_validates_required_instruments(self, mock_config, mock_components):
        ...
        mock_config["component_config"]["position_monitor"][
            "position_subscriptions"
        ] = [
            "binance:BaseToken:BTC"
            # Missing wallet:BaseToken:BTC
        ]

        with pytest.raises(
            ValueError,
            match="Required instrument wallet:BaseToken:BTC not in position_subscriptions",
        ):
            MLBTCDirectionalBTCMarginStrategy(
                config=mock_config,
                data_provider=mock_components["data_provider"],
    ...
    def test_init_success_with_valid_config(self, strategy):
        ...
        assert strategy.entry_instrument == "wallet:BaseToken:BTC"
        assert strategy.perp_instrument == "binance:Perp:BTCUSDT"
        assert len(strategy.available_instruments) == 2


class TestMLBTCDirectionalBTCMarginStrategyActions:
    ...
    def test_create_entry_full_orders(self, strategy):
        ...
        with patch.object(strategy, "_get_asset_price", return_value=50000.0):
            with patch.object(
                strategy,
                "_get_ml_signal",
                return_value={"signal": "LONG", "confidence": 0.8, "sd": 0.02},
            ):
                orders = strategy._create_entry_full_orders(1.0, "long")
                ...
                assert order.amount == 1000.0  # 1.0 * 1000 (leverage)
    ...
    def test_create_entry_partial_orders(self, strategy):
        ...
        with patch.object(strategy, "_get_asset_price", return_value=50000.0):
            with patch.object(
                ...
                orders = strategy._create_entry_partial_orders(0.5, "long")
...
class TestMLBTCDirectionalBTCMarginStrategySLTP:
    ...
    def test_calculate_stop_loss_take_profit_long(self, strategy):
        ...
        result = strategy._calculate_stop_loss_take_profit(
            entry_price, signal_data, "LONG"
        )
    ...
    def test_sd_floor_cap(self, strategy):
        ...
        result_floor = strategy._calculate_stop_loss_take_profit(
            entry_price, signal_data_floor, "LONG"
        )
        assert result_floor["sd_used"] == 0.001  # 10 bps = 0.1%

        # Test SD cap (very large SD)
        signal_data_cap = {"sd": 0.2}  # 20% - should be capped to 10%
        result_cap = strategy._calculate_stop_loss_take_profit(
            entry_price, signal_data_cap, "LONG"
        )
        ...
        result_normal = strategy._calculate_stop_loss_take_profit(
            entry_price, signal_data_normal, "LONG"
        )
...
class TestMLBTCDirectionalBTCMarginStrategyHelpers:
    """Test helper methods."""
    ...
    def test_calculate_target_position(self, strategy):
        ...
        with patch.object(strategy, "_get_asset_price", return_value=50000.0):
            ...
            assert target["btc_perp_position"] == 1000.0  # 1.0 * 1000 (leverage)
            assert target["usdt_balance"] == -999.0
...
class TestMLBTCDirectionalBTCMarginStrategyErrorHandling:
    ...
    def test_get_asset_price_error_handling(self, strategy):
        ...
        with patch.object(
            strategy, "_get_asset_price", side_effect=Exception("Price fetch failed")
        ):
            orders = strategy._create_entry_full_orders(1.0, "long")
    ...
    def test_get_ml_signal_error_handling(self, strategy):
        ...
        with patch.object(
            strategy, "_get_ml_signal", side_effect=Exception("ML signal failed")
        ):
            orders = strategy._create_entry_full_orders(1.0, "long")
