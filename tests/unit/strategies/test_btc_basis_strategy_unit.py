...
@pytest.fixture
def mock_config():
    """Mock configuration for BTC Basis strategy."""
    return {
    'mode': 'btc_basis',
    'share_class': 'USDT',
    'max_leverage': 1.0,   # No leverage for basis trading
    'btc_allocation': 0.8,  # 80% allocation to BTC
    'component_config': {
            'position_monitor': {
...
class TestBTCBasisStrategyInit:
    ...
    def test_init_validates_required_instruments(self, mock_config, mock_components):
        ...
        mock_config['component_config']['position_monitor']['position_subscriptions'] = [
            'wallet:BaseToken:USDT',
            'binance:BaseToken:BTC'
            # Missing binance:Perp:BTCUSDT
        ]
        
        with pytest.raises(ValueError, match="Required instrument binance:Perp:BTCUSDT not in position_subscriptions"):
            BTCBasisStrategy(
                config=mock_config,
                data_provider=mock_components['data_provider'],
                exposure_monitor=mock_components['exposure_monitor'],
    ...
    def test_init_validates_instruments_in_registry(self, mock_config, mock_components):
        ...
        mock_config['component_config']['position_monitor']['position_subscriptions'] = [
            'wallet:BaseToken:USDT',
            'binance:BaseToken:BTC',
            'binance:Perp:BTCUSDT',  # Required instrument
            'invalid:format:BTCUSDT'  # Invalid format
        ]
    ...
    def test_init_success_with_valid_config(self, strategy):
        ...
        assert strategy.spot_instrument == 'binance:BaseToken:BTC'
        assert strategy.perp_instrument == 'binance:Perp:BTCUSDT'
        assert len(strategy.available_instruments) == 3


class TestBTCBasisStrategyActions:
    ...
    def test_generate_orders_entry_full(self, strategy):
        ...
        exposure = {'positions': {}, 'equity': 50000.0}
    ...
    def test_create_entry_full_orders(self, strategy):
        ...
        with patch.object(strategy, '_get_asset_price', return_value=50000.0):
            orders = strategy._create_entry_full_orders(50000.0)
            ...
            assert spot_order.pair == 'BTCUSDT'
            assert spot_order.side == 'BUY'
            assert spot_order.amount == 1.0  # 50000 USDT / 50000 BTC price = 1 BTC
            ...
            assert spot_order.target_token == 'BTC'
    ...
    def test_create_entry_partial_orders(self, strategy):
        ...
        with patch.object(strategy, '_get_asset_price', return_value=50000.0):
            orders = strategy._create_entry_partial_orders(25000.0)
            
            assert len(orders) == 2  # Spot buy + perp short
            for order in orders:
                assert order.strategy_intent == 'entry_partial'
                assert order.amount == 0.5  # 25000 USDT / 50000 BTC price = 0.5 BTC
    ...
    def test_create_exit_full_orders(self, strategy):
        ...
        strategy.position_monitor.get_current_position.return_value = {
            'binance:BaseToken:BTC': 1.0,  # Spot position
            'binance:Perp:BTCUSDT': -1.0   # Perp position (short)
        }
...
class TestBTCBasisStrategyHelpers:
    """Test helper methods."""
    ...
    def test_calculate_target_position(self, strategy):
        ...
        with patch.object(strategy, '_get_asset_price', return_value=50000.0):
            ...
            assert target['btc_balance'] == 0.8  # 50000 USDT * 0.8 allocation / 50000 BTC price = 0.8 BTC
            assert target['btc_perpetual_short'] == -0.8  # Short position
            assert target['usdt_balance'] == 50000.0  # Current equity
    
    def test_get_asset_price(self, strategy):
        """Test _get_asset_price method."""
        price = strategy._get_asset_price()
        assert price == 50000.0  # Mock price from implementation
...
class TestBTCBasisStrategyErrorHandling:
