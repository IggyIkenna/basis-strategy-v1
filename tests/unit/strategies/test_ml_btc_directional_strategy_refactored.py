"""
Unit tests for refactored MLBTCDirectionalStrategy using Order model.
...
from backend.src.basis_strategy_v1.core.strategies.ml_btc_directional_strategy import MLBTCDirectionalStrategy
from backend.src.basis_strategy_v1.core.models.order import Order, OrderOperation


class TestMLBTCDirectionalStrategyRefactored:
    """Test refactored MLBTCDirectionalStrategy with Order model."""
    ...
    def strategy(self, strategy_config, mock_dependencies):
        """Create MLBTCDirectionalStrategy instance."""
        risk_monitor, position_monitor, event_engine, data_provider = mock_dependencies
        return MLBTCDirectionalStrategy(strategy_config, risk_monitor, position_monitor, event_engine, data_provider)
    ...
    def test_make_strategy_decision_neutral_signal_with_position(self, strategy, mock_dependencies):
        ...
        timestamp = pd.Timestamp('2024-01-01 12:00:00')
        market_data = {'prices': {'BTC': 45000.0}}
        exposure_data = {'total_exposure': 10000.0, 'positions': {'btc_perp_position': 0.5}}
        risk_assessment = {}
        
        # Mock existing position
        position_monitor.get_current_position.return_value = {'btc_perp_position': 0.5}
    ...
    def test_create_entry_full_orders_long(self, strategy):
        ...
        assert order.metadata['signal_threshold'] == 0.65
    ...
    def test_create_exit_full_orders_long_position(self, strategy, mock_dependencies):
        ...
        assert order.strategy_intent == 'exit_full'
        assert order.metadata['close_position'] is True
        assert order.metadata['original_position'] == 0.5
    ...
    def test_create_exit_full_orders_short_position(self, strategy, mock_dependencies):
        ...
        # Mock short position
        position_monitor.get_current_position.return_value = {'btc_perp_position': -0.3}
        ...
        assert order.amount == 0.3
        assert order.metadata['original_position'] == -0.3
    ...
    def test_create_exit_full_orders_no_position(self, strategy, mock_dependencies):
        ...
        # Mock no position
        position_monitor.get_current_position.return_value = {'btc_perp_position': 0.0}
    ...
    def test_create_dust_sell_orders(self, strategy):
        ...
        dust_tokens = {
            'ETH': 2.0,
            'USDT': 1000.0,
            'BTC': 0.1  # Should be ignored (target asset)
        }
        
        orders = strategy._create_dust_sell_orders(dust_tokens)
        
        assert isinstance(orders, list)
        assert len(orders) == 2  # ETH and USDT, but not BTC
        
        # Check ETH order
        eth_order = next((o for o in orders if o.pair == 'ETH/BTC'), None)
        ...
        # Check USDT order
        usdt_order = next((o for o in orders if o.pair == 'USDT/BTC'), None)
        assert usdt_order is not None
        assert usdt_order.side == 'SELL'
        assert usdt_order.amount == 1000.0
    ...
    def test_should_exit_for_risk_management(self, strategy):
        ...
        current_price = 45000.0
        current_positions = {'btc_perp_position': 0.5}
        ...
        low_price = stop_loss - 1000  # Below stop loss
        ...
        high_price = take_profit + 1000  # Above take profit
    ...
    def test_position_detection_methods(self, strategy):
        ...
        # Test long position
        long_positions = {'btc_perp_position': 0.5}
        ...
        # Test short position
        short_positions = {'btc_perp_position': -0.3}
        ...
        # Test no position
        no_positions = {'btc_perp_position': 0.0}
    ...
    def test_calculate_target_position(self, strategy):
        ...
        expected_btc_position = equity * strategy.max_position_size
        assert target['btc_perp_position'] == expected_btc_position
        assert target['usdt_balance'] == equity - expected_btc_position
    ...
    def test_get_asset_price(self, strategy):
        ...
        assert price > 0
        assert price == 45000.0  # Mock price
    
    def test_strategy_initialization(self, strategy_config, mock_dependencies):
        """Test strategy initialization."""
        risk_monitor, position_monitor, event_engine, data_provider = mock_dependencies
        
        strategy = MLBTCDirectionalStrategy(strategy_config, risk_monitor, position_monitor, event_engine, data_provider)
        
        assert strategy.share_class == 'BTC'
        assert strategy.asset == 'BTC'
        assert strategy.signal_threshold == 0.65
    ...
    def test_strategy_initialization_missing_config(self, mock_dependencies):
        ...
        with pytest.raises(KeyError):
            MLBTCDirectionalStrategy(incomplete_config, risk_monitor, position_monitor, event_engine, data_provider)
    ...
    def test_get_strategy_info(self, strategy):
        ...
        assert info['strategy_type'] == 'ml_btc_directional'
        assert info['signal_threshold'] == 0.65
        assert info['max_position_size'] == 1.0
        assert info['stop_loss_pct'] == 0.05
        assert info['take_profit_pct'] == 0.15
