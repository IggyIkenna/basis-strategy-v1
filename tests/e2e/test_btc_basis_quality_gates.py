...
class BTCBasisQualityGates:
    ...
    def __init__(self):
        ...
        self.config = self.config_manager.get_complete_config(mode='btc_basis')
    ...
    async def _test_end_to_end_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add missing required configuration for BTC basis strategy
            if 'funding_threshold' not in test_config:
                test_config['funding_threshold'] = 0.0001  # 0.01% funding threshold
            if 'max_leverage' not in test_config:
                test_config['max_leverage'] = 1.0  # No leverage for BTC basis
